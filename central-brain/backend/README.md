...
`protocol_bindings.py` 承载当前 Protocol Binding 注册表，包括 REST active prototype、Android Binder/AIDL service stub sample、带 shared SOA Runtime & Governance precheck + local fallback 的 Linux IPC active sample、Linux gRPC contract skeleton 和 MQTT/SOME-IP/DDS 计划态。
