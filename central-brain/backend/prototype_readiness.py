...
PROTOTYPE_MODULES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "yellow_sun_portable": True,
        "current_state": "empty-interface-registry-with-activation-checklist-and-owner-decision-status-contract",
        "ready_for": ["interface ownership review", "hardware activation checklist review", "hardware owner decision status review", "future HAL/vendor SDK scoping", "no-hardware smoke validation"],
        "not_ready_for": ["real PCIe NPU runtime", "DMA/IOMMU access", "Safety Runtime shared-memory bridge"],
        "android_primary_surface": "getHardwareInterfacesJson, getHardwareInterfaceActivationChecklistJson, getHardwareInterfaceOwnerDecisionStatusJson",
        "linux_sync_surface": "hardware-interfaces, hardware-interface-activation-checklist, and hardware-interface-owner-decision-status over CLI/IPC/gRPC",
    ...
    def readiness_payload(self) -> dict[str, Any]:
        ...
        return {
            ...
            "next_increment_candidates": [
                {
                    "candidate": "hardware interface owner decision evidence intake envelope",
                    "reason": "HW-002/KH owner decision status is visible; the next safe step is a no-hardware reference envelope for attaching owner and smoke evidence without storing files, closing gates, or activating hardware.",
