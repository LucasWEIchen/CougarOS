...
- App 层先绑定 Binder service，再通过 Binder 调用 Uni Info Bus State 与 AI SDK/Agent task plan。
- Binder service sample 默认以上游 prototype gateway `http://10.0.2.2:8787` 访问 mock 后端。
- 用于验证 Android App -> Binder -> AI SDK/Uni Info Bus/SOA semantic gateway 的最小闭环。
