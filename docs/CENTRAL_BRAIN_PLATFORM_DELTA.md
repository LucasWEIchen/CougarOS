...
| Protocol Binding | Binder/AIDL service stub sample；REST 只是 prototype binding；Binder contract 暴露 `/soa/contracts`、shared governance backend target、migration readiness 和 deployment plan | Unix socket IPC active sample；gRPC/RPC JSON contract sample；REST prototype binding；systemd unit hardening check + package profile check；IPC/gRPC 均可查询 `/soa/contracts`、backend contract、migration readiness 和 deployment plan | Android stub + Linux active samples；当前 gRPC/RPC sample 因环境无 `grpcio` 使用 JSON TCP wrapper | XSC-003, XSC-006, NV-P-002, NV-P-003, NV-P-005 |
