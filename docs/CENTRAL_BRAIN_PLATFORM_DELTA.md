...
| --- | --- | --- | --- | --- |
| 应用入口 | Android Console APK 和后续 AI SDK client；Console 已通过 Binder 暴露 `planAgentTaskJson`、`executeAgentTaskJson`、`invokeSkillJson`、`queryMemoryJson` | CLI/client，无 UI 最低样例；CLI/IPC 暴露 `agent-plan`、`agent-execute`、`skill-invoke`、`memory-query` | Console + CLI 初版；AI SDK/Agent plan + execute/Skill/Memory contract mock | DEL-001, DEL-002, XSC-001, APP-004, FW-U-006 |
...
- Linux systemd unit 是带最小 hardening 约束的部署样例，不等同量产包管理、LSM 策略或安全认证基线。
