...
| --- | --- | --- | --- | --- |
| 应用入口 | Android Console APK 和后续 AI SDK client；Console 已通过 Binder 暴露 `planAgentTaskJson`、`executeAgentTaskJson`、`invokeSkillJson`、`queryMemoryJson`、`precheckGovernanceJson`、`getGovernanceDeploymentPlanJson`、`getDriverHalGapsJson` | CLI/client，无 UI 最低样例；CLI/IPC 暴露 `agent-plan`、`agent-execute`、`skill-invoke`、`memory-query`、`governance-precheck`、`governance-deployment-plan`、`driver-gaps` | Console + CLI 初版；AI SDK/Agent plan + execute/Skill/Memory contract mock；Governance precheck、deployment plan 与 Driver/HAL gap backlog 可见 | DEL-001, DEL-002, XSC-001, XSC-004, XSC-005, APP-004, FW-U-006 |
...
| Protocol Binding | Binder/AIDL service stub sample；REST 只是 prototype binding；Binder contract 暴露 shared governance backend target、migration readiness 和 deployment plan | Unix socket IPC active sample；gRPC/RPC JSON contract sample；REST prototype binding；systemd unit hardening check + package profile check；IPC/gRPC 均可查询 backend contract、migration readiness 和 deployment plan | Android stub + Linux active samples；当前 gRPC/RPC sample 因环境无 `grpcio` 使用 JSON TCP wrapper | XSC-006, NV-P-002, NV-P-003, NV-P-005 |
