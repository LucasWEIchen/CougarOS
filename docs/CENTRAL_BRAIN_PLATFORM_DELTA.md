...
## EV-AHR Platform Delta

Event subscription activation approval decision owner handoff evidence acceptance closure handoff readiness audit decision rollup 在 Android 与 Linux 上使用同一语义对象和同一 no-store/no-POST/no-queue/gate/broker/no-side-effect 边界。Android 主路径暴露 `getEventSubscriptionActivationApprovalDecisionOwnerHandoffEvidenceAcceptanceClosureReadinessDecisionReviewerAssignmentAuditDecisionRollupClosureHandoffReadinessAuditDecisionRollupJson` 与 Console `Sub ApHReadyR`，由 Binder service sample 代理 `GET /uib/events/subscriptions/activation-evidence/approval-authority-checklist/decision-dry-run/closure-blocker-matrix/owner-handoff-checklist/audit-consistency/decision-rollup/handoff-evidence-readiness-matrix/audit-consistency/acceptance-status/audit-consistency/decision-rollup/closure-readiness-checklist/audit-consistency/decision-rollup/reviewer-assignment-checklist/audit-consistency/decision-rollup/closure-handoff-readiness-summary/audit-consistency/decision-rollup`；Linux 同步路径暴露 CLI `event-subscription-activation-approval-decision-owner-handoff-evidence-acceptance-closure-readiness-decision-reviewer-assignment-audit-decision-rollup-closure-handoff-readiness-audit-decision-rollup`、IPC `uib.events.subscriptions.activation.approval.decision.owner.handoff.evidence.acceptance.closure.readiness.decision.reviewer.assignment.audit.decision.rollup.closure.handoff.readiness.audit.decision.rollup` 和 gRPC/RPC `GetEventSubscriptionActivationApprovalDecisionOwnerHandoffEvidenceAcceptanceClosureReadinessDecisionReviewerAssignmentAuditDecisionRollupClosureHandoffReadinessAuditDecisionRollup`。

该差异面只涉及调用壳层差异：Android 使用 Binder/AIDL 方法名和 Console debug action，Linux 使用 CLI/Unix socket IPC/gRPC JSON contract sample；两端 payload 均固定 `EV-AHR-001..010`、`decision_rollup_complete=true`、`decision_rollup_consistent=true`、`closure_handoff_audit_decision_blocked=true`、`closure_handoff_audit_decision_ready=false`、`assigned_reviewer_count=0`、`unassigned_reviewer_count=10`、`driver_development_triggered=false` 和 `virtualization_development_triggered=false`。Android system service、Linux daemon、Driver/HAL、DDS/high-rate data plane 和虚拟化层均不在本增量开发范围内。

### EV-AHS Android/Linux closure blocker parity

- Android primary path: Binder `getEventSubscriptionActivationApprovalDecisionOwnerHandoffEvidenceAcceptanceClosureReadinessDecisionReviewerAssignmentAuditDecisionRollupClosureHandoffReadinessAuditDecisionRollupClosureBlockerMatrixJson` and Console `Sub ApHReadyB`.
- Linux synchronized path: CLI `event-subscription-activation-approval-decision-owner-handoff-evidence-acceptance-closure-readiness-decision-reviewer-assignment-audit-decision-rollup-closure-handoff-readiness-audit-decision-rollup-closure-blocker-matrix`, IPC `uib.events.subscriptions.activation.approval.decision.owner.handoff.evidence.acceptance.closure.readiness.decision.reviewer.assignment.audit.decision.rollup.closure.handoff.readiness.audit.decision.rollup.closure.blocker.matrix`, and gRPC/RPC `GetEventSubscriptionActivationApprovalDecisionOwnerHandoffEvidenceAcceptanceClosureReadinessDecisionReviewerAssignmentAuditDecisionRollupClosureHandoffReadinessAuditDecisionRollupClosureBlockerMatrix`.
- Both paths expose the same REST prototype contract `GET /uib/events/subscriptions/activation-evidence/approval-authority-checklist/decision-dry-run/closure-blocker-matrix/owner-handoff-checklist/audit-consistency/decision-rollup/handoff-evidence-readiness-matrix/audit-consistency/acceptance-status/audit-consistency/decision-rollup/closure-readiness-checklist/audit-consistency/decision-rollup/reviewer-assignment-checklist/audit-consistency/decision-rollup/closure-handoff-readiness-summary/audit-consistency/decision-rollup/closure-blocker-matrix` and must keep `hardware_accessed=false`, `driver_development_triggered=false`, `virtualization_development_triggered=false`, `review_queue_updated=false`, `gates_closed=false`, and `activation_allowed=false` until target owners and evidence are supplied.

### EV-AE..EV-AHS prototype readiness closure chain parity

- Android primary path: Binder `getPrototypeReadinessJson` exposes `event_subscription_activation_closure_chain_summary` inside `GET /prototype/readiness`.
- Linux synchronized path: CLI `prototype-readiness`, IPC `prototype.readiness.get`, and gRPC/RPC `GetPrototypeReadiness` expose the same `event_subscription_activation_closure_chain_summary` payload.
- Both paths must report `source_range=EV-AE..EV-AHS`, `event_subscription_activation_closure_chain_stage_count=30`, `event_subscription_activation_closure_chain_ready=false`, `first_gate=EV-AE-001`, `last_gate=EV-AHS-010`, no-store/no-POST/no-side-effect consistency, `driver_development_triggered=false`, `virtualization_development_triggered=false`, and `service_dispatch_triggered=false`. This is only a platform binding difference; Android system service, Linux daemon productionization, Driver/HAL, DDS/high-rate data plane, and virtualization remain outside this increment.
