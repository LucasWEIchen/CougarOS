...
| FW-U-003 | Event | 展锐负责 | 事件订阅 | 必须支持订阅/发布模型 | `/uib/events/topics`、`/uib/events/publish`、`/uib/events/recent` active mock；`/uib/events/subscriptions` 只读订阅 lifecycle/cursor/backpressure/governance 占位契约；legacy `/events/*` 兼容 |
...
| NV-P-006 | DDS | 展锐负责/生态合作 | Topic/Context/... | 高频 Topic/Context 订阅预留 DDS | `/uib/events/*` 已建立 Event 语义 contract、bounded recent log 和 `/uib/events/subscriptions` 订阅占位契约，并完成 Android Binder/Linux IPC/gRPC 映射；DDS/SSE/WebSocket/broker 数据面仍为计划态，待高频 topic 环境 |
