...
R2A delivers Gradle application structured AIDL in `central-brain-sdk`. It is not VINTF stable AIDL: the project cannot add a Soong `aidl_interface`, freeze platform API under `aidl_api`, or modify vendor/system build files because the target Android SDK and system image are prebuilt. This accepted temporary limitation is tracked under `DEV-018` and `ISSUE-021`.
...
R2B must publish production and diagnostic interfaces from separate Android Service components with separate signature-level permissions. The production SDK must not request the diagnostic permission by default.
...
- Client SDK links a `DeathRecipient` to the production service Binder. Service death fails all non-terminal client futures with a stable `SERVICE_DIED` error and requires explicit rebind.
...
R2A defines types only. `CentralBrainRuntimeService.onBind()` still returns `null`; no production or diagnostic Binder is published until R2B implements separate signature-permission services. R3 will derive identity from Binder UID/package/signature, never from `AgentTaskRequest` fields.
