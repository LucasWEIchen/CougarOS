...
| Model Runtime Adapter | NV-F-011 | Android native/runtime bridge + NPU runtime interface contract；debug Console `Infer` 与 Client2 右侧半透明悬浮面板经后端 `/ai/infer` 可观察 mock 或 `ollama-simulated-npu` 仿真结果，Client2 保持车模全屏并使用 single-flight | Linux runtime bridge + NPU runtime interface contract；CLI `infer`、IPC/gRPC `npu-inference` 经 SOA 可观察 mock 或 Ollama simulated NPU 结果，并同步支持 `CENTRAL_BRAIN_OLLAMA_THINK` | NPU/GPU/Cloud 后端可替换；当前 Ollama 仅是用户态仿真后端，默认本地演示 `think=false` 并提取 UI 可见自然语言，固定 `hardware_accessed=false`、`driver_development_triggered=false`、`virtualization_development_triggered=false`、`production_ready=false`，不关闭 DRV-GAP-001；见 `CENTRAL_BRAIN_NPU_RUNTIME_INTERFACE.md` |
...
CENTRAL_BRAIN_BASE_URL=http://127.0.0.1:8787 python3 central-brain/linux-cli/central_brain_cli.py agent-scenarios
CENTRAL_BRAIN_BASE_URL=http://127.0.0.1:8787 python3 central-brain/linux-cli/central_brain_cli.py agent-scenario-home
...
`GET /soa/extensions/closure-summary` 是 `PY-CL-001` / `FW-S-006` 的只读交付面，供 Android/Linux 座舱域工程师确认 SOA service contract visibility 与 Uni Info Bus extension registry 已经在当前 Python prototype 中形成闭环。
