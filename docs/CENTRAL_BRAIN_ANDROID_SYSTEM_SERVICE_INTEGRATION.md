...
与 Memory query contract mock，并提供 Event subscription lifecycle command and transport readiness contract、Runtime & Governance precheck、shared
...
| 组件 | 当前交付 | Req ID |
| --- | --- | --- |
| Uni Info Bus client path | `getStateJson`、`getContextJson`、`listEventTopicsJson`、`publishEventJson`、`getRecentEventsJson`、`getEventSubscriptionsJson`、`requestEventSubscriptionJson`、`cancelEventSubscriptionJson`、`getEventSubscriptionTransportReadinessJson` | XSC-002, FW-U-001, FW-U-002, FW-U-003, NV-P-006 |
...
`getEventSubscriptionsJson`、`requestEventSubscriptionJson`、`cancelEventSubscriptionJson` 和 `getEventSubscriptionTransportReadinessJson` 只返回 Event subscription lifecycle、cursor、backpressure、governance、request/cancel contract-only 命令、callback/watch transport readiness、broker/cursor/backpressure owner 门禁和 Android/Linux binding parity；这些契约不持久化 subscription，不选择 transport，不注册 callback，不启动 broker、SSE/WebSocket、DDS runtime、高频数据面、Driver/HAL 或虚拟化层；
