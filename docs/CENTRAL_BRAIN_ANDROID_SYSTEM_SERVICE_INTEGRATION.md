...
与 Memory query contract mock，并提供 Event subscription placeholder、Runtime & Governance precheck、shared
...
| 组件 | 当前交付 | Req ID |
| --- | --- | --- |
| Uni Info Bus client path | `getStateJson`、`getContextJson`、`listEventTopicsJson`、`publishEventJson`、`getRecentEventsJson`、`getEventSubscriptionsJson` | XSC-002, FW-U-001, FW-U-002, FW-U-003, NV-P-006 |
...
`getEventSubscriptionsJson` 只返回 Event subscription lifecycle、cursor、backpressure、governance 和 Android/Linux binding parity，占位契约不注册 callback、不启动 broker、SSE/WebSocket、DDS runtime、高频数据面、Driver/HAL 或虚拟化层；
