...
`getHardwareInterfaceOwnerDecisionStatusJson`、`submitHardwareInterfaceOwnerDecisionEvidenceJson`、`getHardwareInterfaceOwnerDecisionEvidenceStatusJson` 和 `getHardwareInterfaceOwnerDecisionEvidenceRetentionChecklistJson` 只读或 no-store 返回 hardware owner、evidence intake/status、retention/closure checklist contract；这些路径不持久化 evidence，不读取 evidence store，不创建 review queue，不创建 delete/export workflow，不分配 owner，不关闭 gate，不激活硬件，不触发 Driver/HAL 或虚拟化开发。
...
| Console 可触发 Governance precheck、Driver/HAL gaps、Hardware IF、HW Gate、HW Owner、HW Evidence、HW EvStatus 和 HW Retain | `bash tools/check_central_brain_binding_artifacts.sh` | XSC-004, XSC-005, HW-002, KH-003, KH-006, KH-007, DEL-005 |
