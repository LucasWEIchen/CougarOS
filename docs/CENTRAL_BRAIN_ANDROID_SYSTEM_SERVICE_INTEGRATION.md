...
与 Memory query contract mock，并提供 Event subscription lifecycle command contract、Runtime & Governance precheck、shared
...
| 组件 | 当前交付 | Req ID |
| --- | --- | --- |
| Uni Info Bus client path | `getStateJson`、`getContextJson`、`listEventTopicsJson`、`publishEventJson`、`getRecentEventsJson`、`getEventSubscriptionsJson`、`requestEventSubscriptionJson`、`cancelEventSubscriptionJson` | XSC-002, FW-U-001, FW-U-002, FW-U-003, NV-P-006 |
...
`getEventSubscriptionsJson`、`requestEventSubscriptionJson` 和 `cancelEventSubscriptionJson` 只返回 Event subscription lifecycle、cursor、backpressure、governance、request/cancel contract-only 命令和 Android/Linux binding parity；这些契约不持久化 subscription，不注册 callback，不启动 broker、SSE/WebSocket、DDS runtime、高频数据面、Driver/HAL 或虚拟化层；
