...
FW-U-003/NV-P-006 Event subscription placeholder contract 增量新增 `GET /uib/events/subscriptions`、Android Binder/AIDL `getEventSubscriptionsJson`、Android Console `Event Subs`、Linux CLI/IPC `event-subscriptions`/`uib.events.subscriptions.get` 和 Linux gRPC/RPC `GetEventSubscriptions` 映射；覆盖 XSC-002、XSC-005、XSC-006、FW-U-003、NV-P-002、NV-P-003、NV-P-006、DEL-001、DEL-002。该增量只暴露订阅 lifecycle、cursor/replay、filter、QoS/backpressure、governance、Android/Linux parity 和 `EV-SUB-001..005` 门禁；`broker_active=false`、`dds_runtime_active=false`、`sse_websocket_active=false`、`high_rate_data_plane_active=false`、`hardware_accessed=false`、`driver_development_triggered=false` 和 `virtualization_development_triggered=false` 是验收条件，不启动订阅 broker、callback/watch、SSE/WebSocket、DDS runtime、高频数据面、Driver/HAL、Safety Runtime、共享内存、车辆总线或虚拟化代码。若目标高频 topic 环境需要 DDS、SOME/IP、TSN/PTP 或共享内存，才按 DRV-GAP-004/DRV-GAP-005 记录最小新增开发量。
