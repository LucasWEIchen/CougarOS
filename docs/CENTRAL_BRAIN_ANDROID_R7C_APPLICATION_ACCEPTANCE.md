...
| `R7C-E-002` | Two sequential accepted Client2 requests | Second request replaces the old compatibility stream; each Session has one sequence-1 event/replay |
...
| `R7C-E-004` | Client2 process force-stop/relaunch | New process reinstalls panel hook, binds Runtime and renders Session snapshot projection |
...
typed SDK cancellation race remains covered by instrumentation. Contract schema 1.1 reflects the Session/Event recovery
semantics; application-layer evidence does not mean scenario/Graph/Effect execution is active.
