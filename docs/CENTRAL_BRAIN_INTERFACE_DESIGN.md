...
`GET /uib/events/subscriptions`、`POST /uib/events/subscriptions/request` 与 `POST /uib/events/subscriptions/cancel` 覆盖 XSC-002、FW-U-003、XSC-005、XSC-006、NV-P-002、NV-P-003、NV-P-006、DEL-001、DEL-002。这组三个接口返回订阅 lifecycle、cursor/replay、filter、QoS/backpressure、Runtime & Governance、transport candidates、`EV-SUB-001..006` 门禁和 Android/Linux binding visibility；Android Binder `getEventSubscriptionsJson`/`requestEventSubscriptionJson`/`cancelEventSubscriptionJson`、Linux CLI `event-subscriptions`/`event-subscribe-request`/`event-subscribe-cancel`、Linux IPC `uib.events.subscriptions.get`/`uib.events.subscriptions.request`/`uib.events.subscriptions.cancel` 与 Linux gRPC/RPC `GetEventSubscriptions`/`RequestEventSubscription`/`CancelEventSubscription` 暴露同一视图。这些接口明确 `subscription_persisted=false`、`broker_active=false`、`callback_registered=false`、`watch_started=false`、`dds_runtime_active=false`、`sse_websocket_active=false`、`high_rate_data_plane_active=false`、`hardware_accessed=false`、`driver_development_triggered=false`、`virtualization_development_triggered=false` 和 `service_dispatch_triggered=false`；它们不是真实订阅 broker，不注册 callback/watch，不启动 SSE/WebSocket 或 DDS runtime，也不访问 Driver/HAL、车辆总线或虚拟化层。
...
当前 FW-U-003 增量已实现 `/uib/events/topics`、`/uib/events/publish`、`/uib/events/recent` active mock、`/uib/events/subscriptions` subscription contract，以及 `/uib/events/subscriptions/request` 和 `/uib/events/subscriptions/cancel` contract-only lifecycle command，并在 Android Binder、Linux IPC、Linux gRPC/RPC contract sample 中建立映射。该实现只验证 Uni Info Bus 事件语义、Req ID、订阅 lifecycle/cursor/backpressure/governance、request/cancel 命令和跨平台 binding，不实现 DDS、高频推送 broker、callback/watch、SSE/WebSocket、真实订阅数据面或 Driver/HAL 数据面；这些仍按 NV-P-006 计划态推进。
