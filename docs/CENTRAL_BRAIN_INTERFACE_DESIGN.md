...
R2B publishes `ICentralBrainRuntime` and `ICentralBrainDiagnostics` from separate signature-protected Service components. `CentralBrainClient` uses an explicit component, narrow package visibility, executor-dispatched callbacks and a service `DeathRecipient`; Runtime task work runs off Binder threads and diagnostic pages remain bounded/read-only. API 33 evidence covers protocol negotiation, completion, duplicate cancel, permission denial and diagnostic paging. R2C still owns process-death, rebind and cancel-completion race instrumentation.
