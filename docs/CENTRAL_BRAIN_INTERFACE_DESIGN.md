...
状态：`REPOSITORY_INTERFACE_BASELINE_READY /
PRODUCTION_ACTIVATION_EXTERNAL_BLOCKED / TARGET_HARDWARE_VALIDATION_OPEN`

## 接口文档收口

本文件定义当前 Android 13 AIOS 的 Java、AIDL、C ABI、厂商用户态桥和运维接口。接口集合已
覆盖全部 139 个最小工作包；缺少 OEM/Vendor 证据的 production adapter 只发布 unavailable
合同，不发布可执行默认实现。
