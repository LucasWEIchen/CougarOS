...
| ISSUE-018 | Uni Info Bus Event subscription | `/uib/events/subscriptions` 已列出订阅 lifecycle、cursor/replay、filter、backpressure、governance 和 Android/Linux binding parity，`POST /uib/events/subscriptions/request` 与 `POST /uib/events/subscriptions/cancel` 已固定 contract-only request/cancel 语义，`GET /uib/events/subscriptions/transport-readiness` 已列出 Android callback、Linux watch、SSE/WebSocket、DDS、broker owner、cursor storage owner 和 backpressure/QoS owner 门禁，`GET /uib/events/subscriptions/decision-matrix` 已把 broker owner、cursor storage owner、backpressure/QoS owner、callback/watch shape 和 transport selection 拆成 `EV-DM-001..007` 决策门禁，但真实订阅 broker owner、持久化 owner、SSE/WebSocket/DDS 选择和高频 QoS 仍未定 | 影响 FW-U-003/NV-P-006 从 contract-only 进入真实事件推送；如果 transport、持久化 owner 与背压策略未确认，容易把 readiness mock 或 decision matrix 误当成量产订阅能力 | 暂用 `GET /uib/events/subscriptions`、request/cancel endpoint、transport-readiness endpoint 和 decision-matrix endpoint 固定 `EV-SUB-001..006`、`EV-TR-001..006`、`EV-DM-001..007` 门禁、`subscription_persisted=false`、`transport_selected=false`、`callback_registered=false`、`watch_started=false`、`broker_active=false` 和 `production_activation_allowed=false`；目标 transport、broker owner、cursor storage、QoS/backpressure 和 Android/Linux callback/watch 形态明确前不实现真实订阅 broker 或 DDS 数据面 | Proposed |
