...
| --- | --- | --- | --- | --- | --- | --- |
| DEV-001 | 当前仍以 REST/HTTP 承载 mock 后端，但 Android/Linux 主路径已改为 `/uib/*` 和 `/soa/*`，REST 作为 prototype Protocol Binding；Android Binder sample 和 Linux IPC sample 也以上游 REST gateway 承载语义层 | FW-U-001..008, FW-S-001..006, NV-P-005, XSC-006 | M0 为快速验证端到端链路；A4/A4.2 已新增 Android Binder service/client sample、Linux Unix socket IPC active sample 和 gRPC contract skeleton，但 Android Console 仍未改为绑定 Binder service | 如果继续让业务代码直接调用 legacy REST endpoint，会绕过语义层；如果误把 Binder/IPC sample 当成量产 gateway，会低估服务拆分工作 | 下一步将 Android Console 从 REST prototype 迁移到 Binder service stub，并把 Android Binder/Linux IPC sample 的上游替换为独立 gateway/daemon | Accepted Temporary |
...
| DEV-006 | Runtime & Governance 已参与 `/soa/invoke` 的 registry/discovery/policy/lifecycle/audit precheck，并新增 `CENTRAL_BRAIN_AUDIT_LOG` JSONL 审计持久化样例；但仍是单进程、无真实限流和多协议服务治理 | NV-G-002, NV-G-004, NV-G-005, NV-G-006, NV-G-007, XSC-005, DEL-002 | A3 先把治理从状态展示推进到 active prototype，本轮补重启后最近审计恢复样例 | 治理能力仍不能约束真实多进程/多协议服务；JSONL 只适合开发/单机集成，不等于量产审计后端 | M2 增加真实 QoS/限流、多进程 registry/discovery、审计轮转/导出，并让 IPC/gRPC binding 共用治理模块 | Accepted Temporary |
...
| DEV-013 | A4 Protocol Binding 目前 Android Binder 是 service stub sample，Linux Unix socket IPC 是 active sample，gRPC 仍是 contract skeleton；Android Binder/Linux IPC 都仍代理到同进程 REST gateway，Event 也只是语义 mock/recent log | XSC-006, NV-P-002, NV-P-003, NV-P-006, DEL-001, DEL-002 | 当前增量先固化跨 SoC 绑定 contract 和语义入口映射，并为 Android Binder 与 Linux 同 SoC IPC 提供可读样例 | 集成工程师可能误以为 Binder/IPC sample 已是完整系统服务、多进程治理运行时或 DDS 事件总线 | `/bindings/detail` 明确 `android-binder-aidl` 为 `service-stub-sample`、`linux-ipc` 为 `active-sample`、gRPC 仍为 `contract-skeleton`、DDS 为计划态；下一步实现 Android Console 绑定路径或 Linux 独立 gateway/daemon | Accepted Temporary |
