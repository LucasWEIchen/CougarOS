...
| DEV-003 | Android Console 主按钮已切到 `planAgentTaskJson`，但 AI SDK 仍是 facade/plan mock，没有真实多模态、模型路由、Skill sandbox、Memory/Privacy 或 Agent execute | APP-004, NV-F-001, NV-F-011, XSC-001, FW-U-006 | 本轮让 Console 通过 Binder 调用 `/agent/plan`，App 层只提交 intent/utterance 并获得 policy-aware task graph；SOA 执行仍作为任务图下游 contract | 集成方可能把 plan mock 当成真实 Agent runtime；执行能力、Skill 安全边界和 Memory/Privacy 尚未验证 | M2 增加 `/agent/execute`、Skill sandbox、Memory/Privacy、真实模型路由，并保持执行经 SOA/Tool/Action 与 Runtime & Governance | Accepted Temporary |
