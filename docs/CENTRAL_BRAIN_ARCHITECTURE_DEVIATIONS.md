...
| DEV-006 | Runtime & Governance 已参与 `/soa/invoke` 的 registry/discovery/policy/lifecycle/QoS/audit precheck，并新增 `CENTRAL_BRAIN_AUDIT_LOG` JSONL 审计持久化样例；Linux IPC 对 `soa.service.invoke` 增加本地 precheck 和可选 `CENTRAL_BRAIN_IPC_AUDIT_LOG`；QoS 当前仍是单进程 fixed-window active prototype | NV-G-002, NV-G-004, NV-G-005, NV-G-006, NV-G-007, XSC-005, DEL-002 | A3 先把治理从状态展示推进到 active prototype；本轮把同一治理模块复用到 Linux IPC binding 边界，验证同 SoC IPC 在转发前也能执行治理 | 治理能力仍不能约束真实多进程/多协议服务；JSONL 只适合开发/单机集成，不等于量产审计后端；后端 gateway 与 IPC daemon 的 QoS 窗口仍是各自进程内状态 | M2 增加跨进程 QoS/限流、真实 registry/discovery、审计轮转/导出，并让 IPC/gRPC/Binder binding 共用同一治理服务或 daemon | Accepted Temporary |
...
| DEV-013 | A4 Protocol Binding 目前 Android Console 已绑定 Binder service stub sample，Linux Unix socket IPC 是 active sample 且对 SOA 调用有本地治理 precheck，gRPC 仍是 contract skeleton；Android Binder 和 Linux IPC 允许路径仍代理到同进程 REST gateway，Event 也只是语义 mock/recent log；Android system/privileged service 仍是文档化集成约束 | XSC-006, NV-P-002, NV-P-003, NV-P-006, DEL-001, DEL-002, DEL-003, DEL-004, XSC-005 | 当前增量先固化跨 SoC 绑定 contract 和语义入口映射，并让 Android debug APK 实际走 Binder client/service 路径；本轮让 Linux IPC 的 `soa.service.invoke` 在转发前执行 Runtime & Governance precheck，作为独立 gateway/daemon 拆分前的小样例 | 集成工程师可能误以为 Binder/IPC sample 已是完整系统服务、多进程治理运行时、DDS 事件总线或目标镜像 sepolicy/签名配置；Linux IPC precheck 与后端 gateway precheck 仍不是共享量产治理服务 | `/bindings/detail` 明确 `android-binder-aidl` 为 `service-stub-sample`、`linux-ipc` 为 `active-sample with SOA governance precheck`、gRPC 仍为 `contract-skeleton`、DDS 为计划态；`CENTRAL_BRAIN_ANDROID_SYSTEM_SERVICE_INTEGRATION.md` 明确 system/privileged service 仍需目标 AAOS 镜像决策；下一步做共享治理 daemon、Linux 独立 gateway/daemon 或 Android 目标镜像适配 | Accepted Temporary |
