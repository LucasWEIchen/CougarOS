...
| --- | --- | --- | --- | --- | --- | --- |
| DEV-001 | 当前仍以 REST/HTTP 承载 mock 后端；Android Console 已改为绑定 `CentralBrainGatewayBinderService`，但 Binder service 与 Linux IPC sample 仍以上游 REST gateway 承载语义层 | FW-U-001..008, FW-S-001..006, NV-P-005, XSC-006 | M0 为快速验证端到端链路；A4/A4.2 已新增 Android Binder service/client sample、Linux Unix socket IPC active sample 和 gRPC contract skeleton；Android Console 经 Binder client 调用 `getStateJson` 与 `planAgentTaskJson`；`/uib/actions/request` 已作为 Action 语义入口映射到 Android Binder/Linux IPC/gRPC contract；Android system/privileged service 集成约束已文档化 | 如果误把 Binder/IPC sample 当成量产 gateway，会低估独立系统服务、权限身份、多进程治理、native gateway、签名和 SELinux 工作；Action mock 只做 Policy 检查，不代表真实车控执行 | 下一步把 Android Binder/Linux IPC sample 的上游替换为独立 gateway/daemon；目标 AAOS 镜像明确后按 `CENTRAL_BRAIN_ANDROID_SYSTEM_SERVICE_INTEGRATION.md` 补 framework/priv-app/sepolicy 集成；真实车控必须接入 Vehicle Signal/ECU Adapter 和 Driver/HAL 后再开放 | Accepted Temporary |
...
| DEV-013 | A4 Protocol Binding 目前 Android Console 已绑定 Binder service stub sample，Linux Unix socket IPC 是 active sample，gRPC 仍是 contract skeleton；Android Binder/Linux IPC 都仍代理到同进程 REST gateway，Event 也只是语义 mock/recent log；Android system/privileged service 仍是文档化集成约束 | XSC-006, NV-P-002, NV-P-003, NV-P-006, DEL-001, DEL-002, DEL-003, DEL-004 | 当前增量先固化跨 SoC 绑定 contract 和语义入口映射，并让 Android debug APK 实际走 Binder client/service 路径；本轮新增 `CENTRAL_BRAIN_ANDROID_SYSTEM_SERVICE_INTEGRATION.md` 固定量产集成约束 | 集成工程师可能误以为 Binder/IPC sample 已是完整系统服务、多进程治理运行时、DDS 事件总线或目标镜像 sepolicy/签名配置 | `/bindings/detail` 明确 `android-binder-aidl` 为 `service-stub-sample`、`linux-ipc` 为 `active-sample`、gRPC 仍为 `contract-skeleton`、DDS 为计划态；`CENTRAL_BRAIN_ANDROID_SYSTEM_SERVICE_INTEGRATION.md` 明确 system/privileged service 仍需目标 AAOS 镜像决策；下一步做 Linux 独立 gateway/daemon 或 Android 目标镜像适配 | Accepted Temporary |
