...
| DEV-003 | Android Console 仍有“Invoke SOA Inference”调试入口，虽已走 `/soa/invoke`；本轮新增 `/ai/sdk/capabilities` 与 `/agent/plan` active mock，但尚未把 Console 主按钮切到 AI SDK task API，也没有真实多模态/模型路由/Skill/Memory 执行 | APP-004, NV-F-001, NV-F-011, XSC-001, FW-U-006 | 本轮先建立 AI SDK/Agent facade 与 policy-aware task graph，Android Binder/Linux IPC/gRPC 已有 contract 映射；保留 SOA 调试入口便于验证底层服务入口 | App 仍可能过早理解模型细节；集成方可能把 plan mock 当成真实 Agent runtime | 下一步让 Android Console 使用 `planAgentTaskJson` 作为主任务入口；M2 增加 `/agent/execute`、Skill sandbox、Memory/Privacy、真实模型路由，并保持执行经 SOA/Tool/Action | Accepted Temporary |
