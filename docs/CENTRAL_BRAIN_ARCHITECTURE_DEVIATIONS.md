...
| DEV-013 | A4 Protocol Binding 目前 Android Binder 是 service stub sample，Linux Unix socket IPC 是 active sample，gRPC 仍是 contract skeleton；Android Binder/Linux IPC 都仍代理到同进程 REST gateway | XSC-006, NV-P-002, NV-P-003, DEL-001, DEL-002 | 当前增量先固化跨 SoC 绑定 contract 和语义入口映射，并为 Android Binder 与 Linux 同 SoC IPC 提供可读样例 | 集成工程师可能误以为 Binder/IPC sample 已是完整系统服务或多进程治理运行时 | `/bindings/detail` 明确 `android-binder-aidl` 为 `service-stub-sample`、`linux-ipc` 为 `active-sample`、gRPC 仍为 `contract-skeleton`；下一步实现 Android Console 绑定路径或 Linux 独立 gateway/daemon | Accepted Temporary |
