...
| --- | --- | --- | --- | --- | --- | --- |
| DEV-001 | 当前仍以 REST/HTTP 承载 mock 后端，但 Android/Linux 主路径已改为 `/uib/*` 和 `/soa/*`，REST 作为 prototype Protocol Binding；Linux IPC sample 也以上游 REST gateway 承载语义层 | FW-U-001..008, FW-S-001..006, NV-P-005, XSC-006 | M0 为快速验证端到端链路；A4/A4.1 已新增 Binder/IPC/gRPC contract skeleton 和 Linux Unix socket IPC active sample，但 Android 仍未实现真实 Binder service | 如果继续让业务代码直接调用 legacy REST endpoint，会绕过语义层；如果误把 IPC sample 当成量产 gateway，会低估服务拆分工作 | 下一步将 Android Console 从 REST prototype 迁移到 Binder service stub，并把 Linux IPC sample 的上游替换为独立 gateway/daemon | Accepted Temporary |
...
| DEV-013 | A4 Protocol Binding 目前 Android Binder 和 gRPC 仍是 contract skeleton；Linux Unix socket IPC 已有 active sample，但仍代理到同进程 REST gateway | XSC-006, NV-P-002, NV-P-003, DEL-001, DEL-002 | 当前增量先固化跨 SoC 绑定 contract 和语义入口映射，并为 Linux 同 SoC IPC 提供可运行样例 | 集成工程师可能误以为 Binder/gRPC 已可部署，或误把 IPC sample 当成完整多进程治理运行时 | `/bindings/detail` 明确 `linux-ipc` 为 `active-sample`、Binder/gRPC 仍为 `contract-skeleton`；下一步实现 Android Binder service stub 或 Linux 独立 gateway/daemon | Accepted Temporary |
