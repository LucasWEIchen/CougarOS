...
| A6 | Kernel/HAL/NPU 设计落地 | Driver/HAL/NPU runtime design、PCIe 接入路径 | NPU runtime interface + driver gap backlog + hardware empty-interface registry contract + hardware activation checklist + owner decision status + owner evidence intake/status/retention-closure/replacement-trigger checklist |
