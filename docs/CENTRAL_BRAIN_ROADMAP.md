...
| A8 | 应用层扩展 | 座舱、Agent、Cluster/TBOX、ADAS、诊断视图 | AI SDK/Agent plan + execute/Skill/Memory contract mock + Android Console Binder debug path + Client2 12 场景 typed Binder 验收面板 |
...
- 新增 `CENTRAL_BRAIN_SOFTWARE_DETAILED_DESIGN.md`：按源码说明 Android SDK/AIDL、三 Binder Service、身份/Capability、Job Supervisor/Scheduler、Room v3 durable workflow、Governance/Effect、Model/Event/Memory/Skill、C ABI/JNI、Demo/Client2、Python gateway、Linux binding 和硬件空接口；同时给出七类增量开发步骤、验证矩阵和 code-review checklist。静态门禁从 Gradle/AIDL/C ABI/Client2/Ollama 源码反查一致性；本增量不接线 production Scheduler/Model/Effect，不改变 hardware/Driver/HAL/virtualization 状态。
...
- 完成 B2 Runtime 集成：`CentralBrainRuntimeApplication` 持有唯一进程级 native handle；Runtime startup log/dumpsys 与 Diagnostic Binder sequence 10 暴露同一 fail-closed snapshot，production Service 不获取 native slot、不触发 dispatch。
- API 33 x86_64 已通过 APK 双 ABI/ELF allowlist、native load/init/query/lease/close、容量与重复释放、Runtime dumpsys、Diagnostic parity、force-stop/PID 重建和 crash-buffer 验证；Runtime APK 版本为 `0.2.0-b2`。
- B2 固定 `native_software_provider_available=false`、`native_vendor_npu_provider_available=false`、`native_runtime_dispatch_enabled=false`、`native_hardware_accessed=false`；后续 B3 已完成模拟器范围公开 API 黑盒预检。
- 完成 B1 Native Runtime：C11 ABI V1、JNI `RegisterNatives`、同步 Java wrapper、严格不可变 health snapshot、容量 lease 和生命周期已实现。
...
- 完成 R7A1 aggregate Runtime acceptance：sequence 9 聚合 typed Binder、Governance、durable workflow 与五类 readiness，明确 core ready、R7/production/hardware blocked。
- 9 项 blocker 覆盖 Client2 Binder、API33 E2E、system owner、Effect/Model/Event/Memory/Skill-Governance production 和 target hardware。JVM/API 33 三路 parity、aggregate/evolution、release/lint 与 3-Service/0-probe isolation通过后进入 R7B。
- R7A1 覆盖 Req ID：`APP-004`、`XSC-001`、`XSC-002`、`XSC-004`、`XSC-005`、`XSC-006`、`FW-U-003`、`FW-U-004`、`FW-U-005`、`FW-U-006`、`FW-U-007`、`FW-U-008`、`NV-F-001`、`NV-F-011`、`NV-F-012`、`NV-G-003`、`NV-G-004`、`NV-G-005`、`NV-G-006`、`NV-G-007`、`NV-P-002`、`DEL-001`、`DEL-003`、`DEL-004`、`DEL-005`。下一步 R7B Client2 SDK/Binder migration。
- 完成 R6C3 Skill/Governance readiness：immutable snapshot 已接 Runtime startup log、protected dumpsys 和 existing Diagnostic Binder sequence 8，十项 activation blocker 顺序固定。
- Snapshot 只校验 3 Skill ID/9 middleware stage baseline，不构造 runtime/chain、不变更 AIDL/Room。JVM/API 33 三路 parity、aggregate/evolution、release/lint 与 3-Service/0-probe isolation 通过后，R6 software foundation 结束并进入 R7。
- R6C3 覆盖 Req ID：`APP-004`、`XSC-001`、`XSC-002`、`XSC-004`、`XSC-005`、`XSC-006`、`FW-U-003`、`FW-U-006`、`FW-U-007`、`FW-U-008`、`NV-F-001`、`NV-F-012`、`NV-G-003`、`NV-G-005`、`NV-G-006`、`NV-G-007`、`NV-P-002`、`DEL-001`、`DEL-004`、`DEL-005`。下一步 R7 integration/acceptance。
...
- 完成 R6B2 Memory runtime readiness：immutable snapshot 已接 Runtime startup log、protected dumpsys 和 existing Diagnostic Binder page，八项 durable Memory prerequisite blocker 顺序固定。
- Snapshot 只校验 R6B1 scope/TTL baseline，不构造 lifecycle、不打开 Room/Keystore、不变更 AIDL。JVM/API 33 三路 parity、aggregate/evolution static gate、release build/lint 与 3-Service/0-probe isolation 通过；production activation/storage/key/consent/revocation/clock/repository/middleware 均 false。
- R6B2 覆盖 Req ID：`XSC-001`、`XSC-004`、`XSC-005`、`XSC-006`、`FW-U-006`、`FW-U-007`、`NV-F-001`、`NV-F-012`、`NV-G-005`、`NV-G-006`、`NV-G-007`、`NV-P-002`、`DEL-001`、`DEL-004`、`DEL-005`。下一步 R6C1 signed built-in Skill contract。
...
- 完成 R5B1 deterministic stub provider：TEST_ONLY COLD/READY lifecycle、单 slot async ordered stream、cancel acknowledgement、bounded metrics/history 和 retryable/terminal/fault-isolated injection 已实现。
- JVM build 与 API 33 probe 已验证跨实例 deterministic output、deadline/slot/close、stream/cancel/metrics/fault。Implementation available 但 profile configured/routing、production Service/Router、Ollama/Vendor/hardware 均 false。
- R5B1 覆盖 Req ID：`APP-004`、`XSC-001`、`XSC-004`、`NV-F-011`、`NV-G-004`、`NV-G-006`、`DEL-001`、`DEL-004`、`DEL-005`。下一步 R5B2 test-only Model Router。
- 完成 R5A2 inference resource scheduler：Runtime-policy effective priority、elapsed deadline、global/per-owner queue/running quota、provider slots、priority/deadline/FIFO 和 owner-isolated cancel/settle 已实现。
- Scheduler 对 queued cancel 本地释放，对 running cancel/deadline 只产生 lease-bound directive，固定 `provider_cancel_invoked=false`；当前 Stub/Vendor profile 均 non-routable，production wiring/dispatch false。JVM build 与 API 33 priority/quota/deadline/cancel probe 已通过。
- R5A2 覆盖 Req ID：`APP-004`、`XSC-001`、`XSC-004`、`NV-F-001`、`NV-F-011`、`NV-G-004`、`NV-G-006`、`DEL-001`、`DEL-004`、`DEL-005`。下一步 R5B deterministic provider/Model Router。
