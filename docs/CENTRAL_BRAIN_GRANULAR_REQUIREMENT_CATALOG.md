...
- **需求追踪**：`APP-001/004`, `S2-HMI-001..004`, `S2-UX-002/003`, `DEL-004`。
...
- **前置输入**：1920x1080 `TuanjieView`、RenderService `DisplayIndex=1`、成对安装且同签的 Client2/RenderService debug APK；车模旋转验收还需要映射到 Unity InputSystem 的真实触摸输入。
- **输出与验收**：请求 1.5 render scale；双区默认 26.5°C、18.0/30.0°C 边界、0.5°C 手动/场景共享状态机；Cold 逐级显示 26.5→27.0→27.5→28.0；滑动改变车模朝向且车门点击有效；无 crash/ANR；RenderService 替换后必须按 Client1/DisplayIndex0→Client2/DisplayIndex1 顺序重建共享渲染栈且不清数据，副屏背景和渲染外观由现场复验。
- **边界与非目标**：render scale 是应用层请求，不等于 GPU/Unity 图形质量标定；动态温区是 HMI 仿真，不访问 Vehicle/VHAL/CAN/Driver-HAL，不修改系统镜像或 `libtuanjie.so`；ADB 合成 swipe 不是物理触摸旋转证据，`FLAG_SECURE` 显示的空 screencap 不是视觉验收证据。
- **代码对应**：Client2/Unity 实现：[CockpitControlCoordinator.java](https://github.com/LucasWEIchen/CougarOS/blob/main/apk-labs/client2-central-brain/bridge/src/com/centralbrain/client2/CockpitControlCoordinator.java#L76-L84)；[CockpitControlCoordinator.java](https://github.com/LucasWEIchen/CougarOS/blob/main/apk-labs/client2-central-brain/bridge/src/com/centralbrain/client2/CockpitControlCoordinator.java#L1256-L1275)；[CockpitControlCoordinator.java](https://github.com/LucasWEIchen/CougarOS/blob/main/apk-labs/client2-central-brain/bridge/src/com/centralbrain/client2/CockpitControlCoordinator.java#L1306-L1335)；[CockpitControlCoordinator.java](https://github.com/LucasWEIchen/CougarOS/blob/main/apk-labs/client2-central-brain/bridge/src/com/centralbrain/client2/CockpitControlCoordinator.java#L1338-L1369)；[CockpitControlCoordinator.java](https://github.com/LucasWEIchen/CougarOS/blob/main/apk-labs/client2-central-brain/bridge/src/com/centralbrain/client2/CockpitControlCoordinator.java#L1446-L1486)；[CockpitControlCoordinator.java](https://github.com/LucasWEIchen/CougarOS/blob/main/apk-labs/client2-central-brain/bridge/src/com/centralbrain/client2/CockpitControlCoordinator.java#L1490-L1520)；[patch_unity_hvac_bundle.py](https://github.com/LucasWEIchen/CougarOS/blob/main/apk-labs/renderservice-central-brain/scripts/patch_unity_hvac_bundle.py#L173-L205)；[patch_unity_hvac_bundle.py](https://github.com/LucasWEIchen/CougarOS/blob/main/apk-labs/renderservice-central-brain/scripts/patch_unity_hvac_bundle.py#L225-L248)；[check_central_brain_unity_native_hvac_seat.sh](https://github.com/LucasWEIchen/CougarOS/blob/main/tools/check_central_brain_unity_native_hvac_seat.sh#L23-L46)；[recover_central_brain_android_client1_render_session.sh](https://github.com/LucasWEIchen/CougarOS/blob/main/tools/recover_central_brain_android_client1_render_session.sh#L82-L110)；[recover_central_brain_android_client1_render_session.sh](https://github.com/LucasWEIchen/CougarOS/blob/main/tools/recover_central_brain_android_client1_render_session.sh#L112-L150)。
- **当前状态**：`PRODUCTION_BOARD_PARTIAL_VERIFIED / ORBIT_PHYSICAL_TOUCH_RETEST_OPEN`。
