...
- 边界：只消费 P5-W01 immutable manifest/digest/schema，不重新解释 input/output；不接 Runtime/Graph/Binder/Room，不注册
  ...
  `P5-W03 ToolRuleSolver`；`implementation_stage=P8-W01`，tracking `DEV-064`、`ISSUE-037`。
...
- 边界：不调用模型，不接 Runtime/Graph/Binder/Room/Executor，不注册 production Tool，不触发 Effect/vehicle/NPU/network/
  Driver-HAL。下一工作包 `P5-W04 ToolExecutor boundary`；`implementation_stage=P8-W01`，tracking `DEV-065`、`ISSUE-038`。
...
- 边界：不动态装载 package，不启动 subprocess，不实现 OS 虚拟化；approval-required selection 必须拒绝；不接
  ...
  verifier`；`implementation_stage=P8-W01`，tracking `DEV-066`、`ISSUE-039`。
...
- 边界：只消费受信上游提供的 digest evidence，不读取 APK/JAR/dex/certificate/PackageManager/keystore/TEE，不验证签名链，
  ...
  `P5-W06 WorkingMemoryStore`；`implementation_stage=P8-W01`，tracking `DEV-067`、`ISSUE-040`。
...
- 边界：只保存 process-local bounded opaque bytes，不持久化、不记录内容、不发布给 model context，不接 Runtime/Graph/Binder/
  ...
  encryption/retention owner 未发布。下一工作包 `P5-W07 ProfileMemoryStore`；`implementation_stage=P8-W01`，tracking
  `DEV-068`、`ISSUE-041`。
...
- 边界：main 仅提供 process-local contract-test factory 与 encryption owner gate；不提供 production consent/revocation authority、
  ...
  `implementation_stage=P8-W01`，tracking `DEV-069`、`ISSUE-042`。
...
- 边界：只提供 process-local contract-test factory；不提供 production scenario catalog、storage/read/erase authority、durable/encrypted
  ...
  `implementation_stage=P8-W01`，tracking `DEV-070`、`ISSUE-043`。
...
- 边界：main API 只接受受信 size/priority metadata，不接受、保存或记录原始文本/byte payload；不调用 tokenizer、summarizer、
  ...
  `P5-W10 Memory consent HMI/API`；`implementation_stage=P8-W01`，tracking `DEV-071`、`ISSUE-044`。
...
- 边界：控制器只更新 process-local HMI projection；不读取或记录 Memory 内容，不修改 production repository，不发布
  ...
  implementation`；`implementation_stage=P8-W01`，tracking `DEV-072`、`ISSUE-045`。
...
- 边界：required 类名中的 `Durable` 不代表 process-death durability；当前 retention/replay 仅 process-local，未与旧
  ...
  Model/NPU/Driver-HAL。下一工作包 `P6-W02 Backpressure/QoS`；`implementation_stage=P8-W01`，tracking
  `DEV-073`、`ISSUE-046`。
...
- 关键边界：`CRITICAL_ACTION_OBSERVATION` 不可 drop/coalesce；满队列、过期和 disconnect 必须返回恢复游标/显式 count，不能静默
  ...
  `implementation_stage=P8-W01`，tracking `DEV-073`、`ISSUE-046`。
...
- 关键边界：输出仅为 digest-only `ScenarioSuggestion`，不含用户/模型文本或 Effect payload，auto-execution/Effect 恒为 false。
  ...
  下一工作包 `P6-W04 Proactive consent/policy`；`implementation_stage=P8-W01`，tracking `DEV-074`、`ISSUE-031`。
...
- 关键边界：HIGH/CRITICAL 在 authority 前禁止通用 grant，并始终返回 explicit approval required。POLICY_ELIGIBLE 只代表进入
  ...
  Model/NPU 或 Driver/HAL。下一工作包 `P6-W05 Context source adapters`；`implementation_stage=P8-W01`，tracking `DEV-075`、
  `ISSUE-031`。
...
- 关键边界：不接 TriggerEngine、Runtime/Graph/Effect、Binder、EventBroker publication、Vehicle service/property、Model/NPU、
  ...
  下一工作包 `P6-W06 Active suggestion UX`；`implementation_stage=P8-W01`，tracking `DEV-076`、`ISSUE-031`。
...
- 边界：HMI 只投影受信上游未来可能提供的 suggestion，不读取用户/模型自由文本，不发布 Trigger/Runtime/Graph/Effect，不调用
  ...
  `P7-W01 ModelRequest/Result v2`；`implementation_stage=P8-W01`，tracking `DEV-077`、`ISSUE-031`。
...
- 边界：不替换旧 ModelProvider，不接 ProviderRegistry/Router/Runtime/Graph，不接受 raw user/model/prompt/vehicle payload，不调用
  ...
  `implementation_stage=P8-W01`，tracking `DEV-078`、`ISSUE-024/044`。
...
- 边界：不动态注册、不提供 route/infer、不构造 Provider instance、不接 Runtime/Router，不访问 network/NPU/vehicle/hardware。
  下一工作包 `P7-W03 PolicyAwareModelRouter`；`implementation_stage=P8-W01`，tracking `DEV-079`、`ISSUE-024`。
...
- 边界：只做 route admission metadata，不实例化或调用 Provider，不消费 raw prompt/result，不访问 network/NPU/vehicle/hardware，
  不接 Runtime/Graph/Effect。下一工作包 `P7-W04 LocalModelProvider`；`implementation_stage=P8-W01`，tracking `DEV-080`、
  `ISSUE-024`。
...
- 边界：release source 不包含实现；固定 catalog 仅设置 `developmentAvailable=true`，production implementation/eligibility/readiness
  ...
  schema`；`implementation_stage=P8-W01`，tracking `DEV-081`、`ISSUE-024`。
...
- 边界：只验证进程内候选；不接 LocalModelProvider/Router/Runtime/Graph/Effect，不执行 repair prompt，不记录 raw output，不访问
  network/NPU/vehicle/hardware。下一工作包 `P7-W06 Scenario evaluation harness`；`implementation_stage=P8-W01`，tracking
  `DEV-082`、`ISSUE-024`。
...
- 边界：offline evaluation only；不调用 Provider/model，不接 Runtime/Graph/Effect/Vehicle/NPU/Driver-HAL，不授予 action authority，
  ...
  `implementation_stage=P8-W01`，tracking `DEV-083`、`ISSUE-024`。
...
- DoD：七项 JVM test 验证前台车辆任务先于后台队列、ELEVATED/CONSTRAINED compact budget、HOT 仅允许 minimal
...
- 边界：只排队 metadata，不调用 Provider/model，不接 Runtime/Graph/Effect，不读取系统 thermal/NPU/VHAL/Driver-HAL；
  ...
  `implementation_stage=P8-W01`，tracking `DEV-084`、`ISSUE-024`。
