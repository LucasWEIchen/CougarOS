...
## P7-R3-OC OpenClaw 目标环境过渡网关

当前目标算力基座只提供 OpenClaw，因此 `P7-R3-OC` 在不改变 `ModelProvider` 上层接口的前提下增加固定
`ws://169.254.208.110:18789/` WebSocket v3 Provider。凭据不写入源码、APK、Room、checkpoint 或日志，只能通过
`DUMP` 保护的 debug Activity 注入 Runtime 进程。回复仍需通过严格 JSON、scenario binding 和 action allowlist，
...
| P7-R3-OC OpenClaw target gateway | `openclaw_target_integration_implemented=true`；`openclaw_target_android13_arm64_verified=true`；`client2_openclaw_projection_verified=true`；`production_provider_qualified=false` | 目标 Android 经固定 WebSocket v3 真实调用外部 OpenClaw 并投影回复；直接 NPU、车辆 Effect 和 release 资格仍为 false |
...
flowchart TB
  ...
  subgraph Runtime["Android AIOS Runtime"]
    ...
    OpenClawGateway["Fixed OpenClaw WebSocket v3 Gateway（P7-R3-OC target verified）"]
...
| P7-R3-OC OpenClaw target gateway | 固定 target WebSocket v3、进程内凭据、challenge/auth、send/abort/history、严格输出、target Router profile、Client2 投影 | API 33 ARM64 Runtime 探针与 Client2 冷场景通过真实 OpenClaw；cleartext/ADB credential/release qualification 仍未关闭 | `TRANSITIONAL TARGET INTEGRATION VERIFIED` |
...
| P7-R3 Model Runtime production | P7-R3-OC 已完成当前 OpenClaw 过渡 target profile、真机模型调用和 Client2 投影；仍需正式 credential/TLS、health/version、artifact owner、release Provider 与 Ollama 迁移 | 目标 NPU 直接证据、可信 resource producer、链路安全和标定 owner 见 `ISSUE-024/044` | `TRANSITIONAL INTEGRATION / EXTERNAL_VALIDATION` |
