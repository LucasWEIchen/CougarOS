...
| Client2 基础演示 HMI 与 Demo HMI | 导航触发悬浮菜单、四项自然场景、Intent/Plan/Execution/Result、typed Session/Event、immutable reducer、Java lifecycle、HVAC controls、text-free process resume | Android 13 ARM64 应用层；四阶段 shell 和 HVAC control surface 已完成，Seat 与 Effect 执行未接 | `DEVELOPED` |
...
| 模块 | 最小剩余工作 | 阻塞或下一步 | 状态 |
| --- | --- | --- | --- |
| 场景解析与仿真编排 | P2 foundation、P3-W01..W09、P4-W01..W04 已完成；仍需 Seat 控制页与 Runtime 场景编排闭环 | Stage 2 P4-W05 Seat control surface | `IN_PROGRESS` |
...
| 中控 AIOS 演示闭环 | HVAC/Seat/Media/Nav Effect 详情中，四阶段 shell 和 HVAC 手动受理已完成；继续实现 Seat/Media/Nav、手动和 AI 共用执行链及真实 readback 投影 | Stage 2 P4-W05..W10；`S2-HMI-001..006` | `IN_PROGRESS` |
...
| `apk-labs/client2-central-brain/` | 座舱演示 HMI | `Client2ScenarioBridge`、immutable `CockpitHmiState/CockpitHvacState/HvacControlIntent`、唯一 reducer、Java coordinator、四阶段 overlay、HVAC controls 与 APK patch/build/test；下一步增加 Seat control surface |
