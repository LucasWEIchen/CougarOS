...
flowchart TB
  ...
  TriggerEngineP6 -. "consent / policy + Runtime publication pending" .-> ScenarioResolver
...
P6-W01..P6-W03 状态：`event_broker_interface_defined=true`；`event_broker_typed_topics_verified=true`；
...
| Proactive consent/policy | P6-W03 已形成 process-local suggestion-only TriggerEngine；仍需可信 source adapter、主动建议同意/策略、durable cooldown 与 Runtime publication | 下一步 P6-W04；产品/隐私 owner 见 `ISSUE-031` | `IN_PROGRESS` |
...
| `central-brain/android-runtime/runtime-service/src/main/java/com/centralbrain/runtime/events` | Event/Trigger contract | R6 bounded runtime/readiness、P6-W01 typed EventBroker、P6-W02 per-subscription Backpressure/QoS 与 P6-W03 suggestion-only TriggerEngine；可信 source、durable cooldown、consent/policy、broker composition 和 production Runtime 未接 |
