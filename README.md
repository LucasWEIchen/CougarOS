...
| 模块 | 最小剩余工作 | 阻塞或下一步 | 状态 |
| --- | --- | --- | --- |
| 场景解析与仿真编排 | P2 foundation、P3-W01..W09、P4-W01..W05 已完成；仍需 Runtime 场景编排与 Effect timeline 闭环 | Stage 2 P4-W06 Plan/Effect execution timeline | `IN_PROGRESS` |
...
| 中控 AIOS 演示闭环 | HVAC/Seat/Media/Nav Effect 详情中，四阶段 shell、HVAC 和 Seat 手动受理已完成；继续实现执行 timeline、Media/Nav、手动和 AI 共用执行链及真实 readback 投影 | Stage 2 P4-W06..W10；`S2-HMI-001..006` | `IN_PROGRESS` |
...
| `apk-labs/client2-central-brain/` | 座舱演示 HMI | `Client2ScenarioBridge`、immutable `CockpitHmiState/CockpitHvacState/CockpitSeatState`、`HvacControlIntent/SeatControlIntent`、唯一 reducer、Java coordinator、四阶段 overlay、HVAC/Seat controls 与 APK patch/build/test；下一步增加 Plan/Effect execution timeline |
