...
| P4-D4c Simulated Scenario Binder | debug-only AIDL v1、Parcelable metadata、固定 Cold/Fatigue x Parked/Moving 输入、签名权限与 capability 双门禁 | 六组 JVM + Debug/Release 构建；Android 13 安装可见且未授权调用被拒；未完成同签名正向 Binder、Client2/adapter/readback | `DEVELOPED` |
...
| 场景与仿真 Effect 编排 | P4-D4a..D4c 已完成 Graph、进程内 projection 与受保护 debug Binder；仍需 P4-D4d adapter/readback composition 与 Client2 wiring | Stage 2 P4；不依赖真实车身信号 | `IN_PROGRESS` |
