...
| 模块 | 最小剩余工作 | 阻塞或下一步 | 状态 |
| --- | --- | --- | --- |
| 场景解析与量产编排 | P4-D4 debug 场景编排和 Client2 仿真闭环已完成；仍需 production Scenario/Graph/Effect publication | P8 真实 adapter 与 P9 owner/发布证据；不得用 debug simulation 隐式关闭 | `IN_PROGRESS` |
...
| 中控 AIOS 量产闭环 | P4-D4e debug 演示已形成自然输入、自动 simulated Effect/readback、显式审批和七阶段展示；真实车辆执行、production approval/retry/undo 尚未发布 | P8 adapter + P9 production owner；`S2-HMI-001..006` | `IN_PROGRESS` |
