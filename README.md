...
| 模块 | 最小剩余工作 | 阻塞或下一步 | 状态 |
| --- | --- | --- | --- |
| 场景解析与仿真编排 | P2 foundation、P3-W01..W09、P4-W01..W06 已完成；HMI timeline 已交付，仍需 Runtime 场景编排与 Effect publication | Stage 2 P4-W07 Approval/partial/retry/undo UX | `IN_PROGRESS` |
...
| 中控 AIOS 演示闭环 | 四阶段 shell、HVAC/Seat 手动受理、七阶段执行 timeline、Media/Nav 状态投影已完成；继续 approval/partial/retry/undo、驾驶限制、工程抽屉和状态同步 | Stage 2 P4-W07..W10；`S2-HMI-001..006` | `IN_PROGRESS` |
...
| `apk-labs/client2-central-brain/` | 座舱演示 HMI | `Client2ScenarioBridge`、immutable `CockpitHmiState/CockpitHvacState/CockpitSeatState/CockpitExecutionTimeline`、控制 targets、唯一 reducer、Java coordinator、四阶段 overlay、HVAC/Seat controls、七阶段 typed-event timeline 与 APK patch/build/test；下一步 P4-W07 approval/partial/retry/undo UX |
