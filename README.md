...
| P9 Parser Robustness Host Campaign | Jazzer 0.30.0、20 秒默认预算、6 seed、checkpoint/scenario/tool 三 surface、执行/覆盖/入口/零崩溃/无 raw-input logging 门禁 | W03f bounded host evidence 已验证；不是 Android Binder/Parcel、目标长预算或量产安全资格 | `DEVELOPED / HOST VERIFIED` |
...
| P9-W03 Security review/fuzz | W03a parser corpus、W03b identity/replay/signer corpus、W03c 37 项 AIDL inventory、W03d Binder identity、W03e callback replay 和 W03f 三类 parser 受控覆盖率引导主机测试已完成 | 仍需安全 owner approval、production signer/release 资格与目标级安全测试；见 `DEV-088..090/111..113`、`ISSUE-050` | `EXTERNAL_BLOCKED` |
...
| 日期 | 提交或版本 | 修改内容 | 状态边界 |
| --- | --- | --- | --- |
| 2026-07-19 | [P9-W03f Parser robustness campaign](central-brain/contracts/central_brain_android_p9_parser_robustness_campaign.json) | 固定 Jazzer 0.30.0、20 秒默认预算、6 个 synthetic seed 和 checkpoint/scenario/tool 三类 Java parser surface；受控运行器校验执行量、覆盖率、入口计数、零崩溃与无 raw-input logging | 主机工程证据完成；`security_coverage_guided_fuzz_complete=false`，production signer、安全 owner、目标级测试与量产资格仍未完成 |
