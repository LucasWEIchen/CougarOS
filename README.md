...
| 模块 | 最小剩余工作 | 阻塞或下一步 | 状态 |
| --- | --- | --- | --- |
| 场景解析与仿真编排 | P2 foundation、P3-W01..W09、P4-W01..W08 已完成；HMI timeline/recovery/restriction 已交付，仍需 Runtime 场景编排与 Effect publication | Stage 2 P4-W09 Engineer simulation drawer | `IN_PROGRESS` |
...
| 中控 AIOS 演示闭环 | 四阶段 shell、HVAC/Seat 手动受理、七阶段执行 timeline、approval/partial/compensation 与 disabled recovery commands 已完成；继续工程抽屉和状态同步 | Stage 2 P4-W09..W10；`S2-HMI-001..006` | `IN_PROGRESS` |
...
| `apk-labs/client2-central-brain/` | 座舱演示 HMI | `Client2ScenarioBridge`、immutable HMI/device/execution/recovery state、`DrivingUxPolicy/PanelPresentationMode`、唯一 reducer、四阶段 overlay、HVAC/Seat controls、七阶段 timeline、approval/recovery/restriction 与 APK patch/build/test；下一步 P4-W09 Engineer simulation drawer |
