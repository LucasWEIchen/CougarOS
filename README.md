...
flowchart TB
  ...
  ModelRouterP7 -->|"deterministic test provider"| DecisionComposition
...
| Model Runtime production | Model V2、registry/health、router、structured output、evaluation、resource/thermal admission 与 debug composition 软件完成 | Vendor NPU/provider、可信 resource producer 与标定 owner 见 `ISSUE-024` | `SOFTWARE_COMPLETE / EXTERNAL_BLOCKED` |
...
仓库内可独立实现的 Android 13 AIOS 软件需求已全部实现或由失败关闭接口覆盖，未分类仓库需求数为 0。
