...
| P9-W06 Driver safety | W06a/W06b 已完成失败关闭准入合同、27-key 脱敏 projection、debug Activity 和只读 adapter | OEM owner、真实 vehicle state、驾驶分心矩阵和座椅硬联锁验收仍由 `DEV-096/097`、`ISSUE-029/030` 外部阻塞；下一软件增量为 P9-W07 | `EXTERNAL_BLOCKED` |
| P9-W07 Release evidence/diagnostics | W07a evidence envelope 与 W07b 31-key debug probe/no-install adapter 已完成 | W07c replacement release/issue-retest workflow 待开发；target probe/owner evidence 见 `DEV-098/099`、`ISSUE-052/053`；下一软件增量为 P9-W07c | `IN_PROGRESS` |
...
| 日期 | 提交或版本 | 修改内容 | 状态边界 |
| --- | --- | --- | --- |
| 2026-07-18 | [P9-W07b Field Diagnostics Probe](docs/CENTRAL_BRAIN_RELEASE_EVIDENCE_FIELD_DIAGNOSTICS.md) | 新增 31-key projection、DUMP debug Activity、三包/launcher/Service preflight 和 5 executed + 3 NOT_RUN adapter | host software only；目标未执行、report 未 admitted；下一步 P9-W07c |
