...
flowchart TB
  ...
  EventBrokerP6 -. "DDS / Binder / QoS publication pending" .-> GraphRuntime
...
P6-W01 状态：`event_broker_interface_defined=true`；`event_broker_typed_topics_verified=true`；
...
| Event Broker production/QoS | P6-W01 typed/cursor/replay/identity foundation 已完成；仍需 bounded queue、critical no-drop、durable repository 与 middleware publication | 下一步 P6-W02 Backpressure/QoS；production owner 见 `ISSUE-046` | `IN_PROGRESS` |
...
| `central-brain/android-runtime/runtime-service/src/main/java/com/centralbrain/runtime/events` | Event contract | R6 bounded runtime/readiness 与 P6-W01 typed EventBroker/process-local append/replay；未接 durable cursor repository、DDS/SOME-IP 或 production Runtime |
