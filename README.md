...
flowchart TB
  ...
  Client2 --> HmiState --> ScenarioSync --> ClientBridge --> Facade
...
| 模块 | 最小剩余工作 | 阻塞或下一步 | 状态 |
| --- | --- | --- | --- |
| 场景解析与仿真编排 | P2 foundation、P3-W01..W09、P4-W01..W10 已完成；HMI scenario/manual synchronization 已交付，仍需 Runtime 场景编排与 Effect publication | Stage 2 P4-W11 Accessibility/display matrix | `IN_PROGRESS` |
...
| 中控 AIOS 演示闭环 | 四阶段 shell、HVAC/Seat 手动受理、timeline/recovery/restriction、工程抽屉和场景/手动状态同步已完成；继续完成显示矩阵与聚合验收 | Stage 2 P4-W11；`S2-HMI-001..006` | `IN_PROGRESS` |
...
| `apk-labs/client2-central-brain/` | 座舱演示 HMI | `Client2ScenarioBridge`、immutable HMI/device/execution/recovery/engineer state、`DrivingUxPolicy`、唯一 reducer、四阶段 overlay、HVAC/Seat controls、七阶段执行 timeline、recovery/restriction、protected debug drawer 与 APK patch/build/test；已完成 P4-W10 状态同步；下一步 P4-W11 显示矩阵 |
