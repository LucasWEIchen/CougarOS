...
Client2 的 cold/fatigue 演示已从历史 `ISimulatedScenarioRuntime` debug Binder 迁移到正式
`OrchestrationClient` SDK V1。界面先通过 `SessionClient` 获取 owner-scoped Session ID，再读取已有
Orchestration snapshot；仅在 `ORCHESTRATION_NOT_STARTED` 时启动显式 debug simulation profile。
SDK 同时校验 projection digest，Client2 再读取 typed Plan 并执行 `validatePlanForSnapshot`。疲惫场景的
批准/拒绝绑定当前 `approvalId + projectionDigest`，不被当作可信授权。旧客户端、复制 Parcelable 和
第二条场景执行链已删除；`IDebugSimulationController` 仅继续提供受保护的工程测试 Context。

当前 `client2_orchestration_sdk_v1_wired=true`、`client2_session_before_orchestration=true`、
`client2_orchestration_resume_read_before_start=true`、`client2_orchestration_plan_validated=true`、
`client2_orchestration_approval_projection_bound=true`、`client2_orchestration_debug_capability_policy_wired=true`、
...
`S2-HMI-003/006`、`XSC-001/005/006`、`DEL-001/003/004`；tracking：`DEV-119`、`ISSUE-033`。

## P5-R1 Runtime 组合交付

`P5-R1` 在 P4-R1 的 debug Orchestration backend 内组合既有 Tool manifest/registry/resolver/rules/executor
边界、built-in Skill 治理准入、metadata-only Context Budget 和 session-scoped Working Memory。Cold/Fatigue
分别绑定 `cabin.precondition`/`cabin.scene.nap`；组合摘要进入 Working Memory，并绑定到节点和 Effect 的
evidence digest，终态或 Service 关闭时清理。该路径只存在于 debug source set，不写 Profile/Episodic Memory，
不接收自由文本，也不启用 Skill dispatch、量产 Tool execution、网络、NPU、车辆或 Driver/HAL。

当前 `runtime_composition_debug_wired=true`、`runtime_composition_host_tests_verified=true`、
...
`P6-P7-R1` 在同一 debug Orchestration backend 中新增摘要化决策链：Runtime health、时间和冷舱模拟信号先经
Context Source Adapter 规范化；Cold/Fatigue 固定规则经持续窗口只产生 suggestion；Proactive Consent 在无 grant 时明确
拒绝自动执行；Model Registry/Policy Router 仅选择离线 deterministic test provider，并通过受限测试 router 产生输出摘要；
Model health 与 consent decision 以两条 process-local Event 摘要发布、派发并注销。决策摘要与 P5 Runtime 摘要共同绑定到
Node/Effect evidence。Fatigue DMS 来源明确为 build-owned debug stub；release、真实模型/NPU、生产 Event transport、车辆均未接入。

当前 `decision_composition_debug_wired=true`、`decision_composition_host_tests_verified=true`、
...
## P7-R2 WSL Ollama 开发网关

`P7-R2` 将 debug Orchestration 的模型路径从 deterministic test provider 升级为真实 HTTP 调用：Android Runtime
只访问 `127.0.0.1:11434`，由 `adb reverse tcp:11434 tcp:11434` 转发到 WSL Ollama，并运行
`qwen3.5:27b-optimized`。响应必须通过固定 JSON Schema、scenario binding、reply bound 和 action allowlist；模型
回复只通过当前 Binder snapshot 投影到 Client2，不写 Room、checkpoint 或日志。量产端点同时冻结为
`169.254.208.110:11434`，但 release Provider 保持禁用且模型名为 `UNCONFIGURED`。

API 33 ARM64 实机已验证模型真实加载和终态响应，单次 Fatigue 推理 43,739 ms，Ollama 观测为
`90%/10% CPU/GPU`。这证明开发模型链，不是目标 NPU 或车辆 Effect 证据。当前
...
详见 [Ollama Model Gateway 详细设计](docs/CENTRAL_BRAIN_OLLAMA_MODEL_GATEWAY.md)；Req IDs：
`S2-MDL-001`、`S2-SAF-001`、`S2-OBS-001`、`XSC-001/005/006`、`DEL-001/003/004/005`；
tracking：`DEV-121`、`ISSUE-024/044`。

## P7-R3-OC2 OpenClaw 目标环境过渡网关

当前目标算力基座只提供 OpenClaw，因此 `P7-R3-OC2` 在不改变 `ModelProvider` 上层接口的前提下增加固定
`ws://169.254.208.110:18789/` WebSocket v3 Provider。按维护者明确指令，完整控制页 URL 和 token 固化在
`OpenClawEndpointConfig` 并随 APK 分发；不再保留 ADB 凭据注入 Activity/Store。回复仍需通过严格 JSON、scenario binding 和 action allowlist，
并只经 owner/session-scoped debug Binder 投影到 Client2；模型没有 Effect 权限。

Android 13 ARM64 真机已验证 challenge/auth、`chat.send` ACK、结构化终态、Runtime 探针和 Client2 UI 投影。
这证明目标网络中的真实外部模型调用，不证明直接 NPU API、车辆控制或量产发布资格。当前
`openclaw_target_integration_implemented=true`、`openclaw_target_android13_arm64_verified=true`、
`client2_openclaw_projection_verified=true`、`external_compute_accessed=true`、`direct_npu_accessed=false`、
...
| P5 Tool Manifest/Schema | `tool_manifest_contract_defined=true`；`tool_manifest_schema_version=1`；`tool_manifest_contract_digest_verified=true`；`tool_schema_exact_scalar_validation_verified=true`；`tool_manifest_health_fail_closed=true`；`tool_manifest_android13_arm64_verified=true`；`tool_registry_published=false`；`tool_resolver_published=false`；`tool_execution_enabled=false`；`production_tool_artifact_loaded=false`；`implementation_stage=P9-W03` | P5-W01 软件合同完成；JVM 和 debug/release compile 已通过；API 33 ARM64 debug probe 已通过 |
| P5 Tool Registry/Resolver | `tool_registry_contract_defined=true`；`tool_resolver_contract_defined=true`；`tool_health_dynamic_snapshot_defined=true`；`tool_registry_digest_verified=true`；`tool_registry_version_conflict_rejected=true`；`tool_resolver_states_separated=true`；`tool_resolver_unhealthy_no_fallback=true`；`tool_registry_android13_arm64_verified=true`；`tool_registry_published=false`；`tool_resolver_published=false`；`tool_registry_runtime_wired=false`；`tool_execution_enabled=false`；`production_tool_registered=false`；`implementation_stage=P9-W03` | P5-W02 pure-Java 合同完成；registered/resolved/usable、最高兼容版本和动态健康失败关闭已验证，API 33 ARM64 debug probe 已通过 |
| P5 Tool RuleSolver | `tool_rule_set_contract_defined=true`；`tool_rule_type_count=6`；`tool_rule_set_digest_verified=true`；`tool_rule_init_child_conditional_verified=true`；`tool_rule_model_intersection_fail_closed=true`；`tool_rule_terminal_requirements_verified=true`；`tool_rule_approval_annotation_fail_closed=true`；`tool_rule_solver_android13_arm64_verified=true`；`tool_rule_solver_published=false`；`tool_rule_solver_runtime_wired=false`；`tool_approval_authority_available=false`；`tool_execution_enabled=false`；`production_tool_registered=false`；`implementation_stage=P9-W03` | P5-W03 pure-Java rule intersection 完成；模型选择不能扩展 allowset，approval 只标记不授权；API 33 ARM64 debug probe 已通过 |
| P5 Tool Executor boundary | `tool_executor_contract_defined=true`；`tool_invocation_context_defined=true`；`built_in_allowlist_enforced=true`；`built_in_signer_artifact_bound=true`；`tool_executor_host_execution_verified=true`；`tool_executor_deadline_cancel_verified=true`；`tool_executor_output_limit_verified=true`；`tool_executor_audit_bounded_verified=true`；`tool_executor_android13_arm64_verified=true`；`tool_executor_runtime_wired=false`；`tool_execution_enabled=false`；`production_tool_execution_enabled=false`；`production_tool_registered=false`；`os_virtualization_enabled=false`；`implementation_stage=P9-W03` | P5-W04 in-process built-in 边界完成；不接 Runtime/Graph/approval，不提供 production signer evidence 或强制线程抢占；API 33 ARM64 debug probe 已通过 |
| P5 Skill package verifier | `skill_artifact_verifier_contract_defined=true`；`skill_signer_policy_contract_defined=true`；`skill_version_policy_contract_defined=true`；`skill_artifact_hash_verified=true`；`skill_manifest_digest_verified=true`；`skill_signer_policy_verified=true`；`skill_runtime_version_verified=true`；`skill_capability_policy_verified=true`；`skill_revocation_downgrade_fail_closed=true`；`skill_package_verifier_android13_arm64_verified=true`；`trusted_skill_evidence_source_configured=false`；`package_signature_cryptographically_verified=false`；`dynamic_skill_loading_enabled=false`；`skill_execution_enabled=false`；`skill_package_verifier_runtime_wired=false`；`implementation_stage=P9-W03` | P5-W05 static verifier 完成；只验证 supplied digest evidence，不读取/加载 package，不构成签名链、量产 trust 或执行 authority；API 33 ARM64 debug probe 已通过 |
| P5 WorkingMemoryStore | `working_memory_store_defined=true`；`working_memory_session_scope_verified=true`；`working_memory_ttl_verified=true`；`working_memory_item_limit_verified=true`；`working_memory_byte_limit_verified=true`；`working_memory_token_limit_verified=true`；`working_memory_terminal_cleanup_verified=true`；`working_memory_payload_zeroized_on_cleanup=true`；`working_memory_android13_arm64_verified=true`；`working_memory_process_local=true`；`working_memory_persistence_wired=false`；`working_memory_runtime_wired=false`；`working_memory_model_context_published=false`；`working_memory_tokenizer_verified=false`；`implementation_stage=P9-W03` | P5-W06 process-local store 完成；保存 bounded opaque bytes 并在 expiry/remove/terminal cleanup 覆零，不接 Runtime/Room/Graph/model/hardware；API 33 ARM64 debug probe 已通过 |
| P5 ProfileMemoryStore | `profile_memory_store_defined=true`；`profile_memory_explicit_consent_verified=true`；`profile_memory_field_allowlist_verified=true`；`profile_memory_user_seat_scope_verified=true`；`profile_memory_read_update_verified=true`；`profile_memory_delete_verified=true`；`profile_memory_export_verified=true`；`profile_memory_consent_revocation_fail_closed=true`；`profile_memory_encryption_owner_gate_verified=true`；`profile_memory_sealed_payload_zeroized=true`；`profile_memory_android13_arm64_verified=true`；`profile_memory_process_local=true`；`profile_memory_durable_storage_wired=false`；`profile_memory_production_encryption_owner_configured=false`；`profile_memory_consent_authority_production_wired=false`；`profile_memory_runtime_wired=false`；`implementation_stage=P9-W03` | P5-W07 contract-test store 完成；固定 typed field/user-seat/consent/auth 与 sealed owner gate，不是 production encryption/Keystore/Room；API 33 ARM64 debug probe 已通过 |
| P5 EpisodicMemoryStore | `episodic_memory_store_defined=true`；`episodic_memory_summary_result_only_verified=true`；`episodic_memory_owner_isolation_verified=true`；`episodic_memory_policy_fail_closed=true`；`episodic_memory_read_fail_closed=true`；`episodic_memory_retention_verified=true`；`episodic_memory_capacity_verified=true`；`episodic_memory_erase_verified=true`；`episodic_memory_erase_fail_closed=true`；`episodic_memory_android13_arm64_verified=true`；`episodic_memory_process_local=true`；`episodic_memory_raw_continuous_signal_stored=false`；`episodic_memory_arbitrary_payload_stored=false`；`episodic_memory_persistence_wired=false`；`episodic_memory_runtime_wired=false`；`episodic_memory_model_context_published=false`；`episodic_memory_production_read_authority_wired=false`；`implementation_stage=P9-W03` | P5-W08 typed summary/result store 完成；不接受原始连续信号、任意 payload 或自由文本，production catalog/policy/read/erase/repository/Runtime 未接；API 33 ARM64 debug probe 已通过 |
| P5 ContextBudgetManager | `context_budget_manager_defined=true`；`context_budget_category_allocation_verified=true`；`context_budget_dual_limit_verified=true`；`context_budget_deterministic_overflow_verified=true`；`context_budget_required_fail_closed=true`；`context_budget_android13_arm64_verified=true`；`context_budget_decision_only=true`；`context_budget_text_payload_accepted=false`；`context_budget_tokenizer_wired=false`；`context_budget_summarizer_wired=false`；`context_budget_production_authority_wired=false`；`context_budget_runtime_wired=false`；`context_budget_content_logged=false`；`implementation_stage=P9-W03` | P5-W09 metadata-only allocator 完成；只生成预算指令，不接 content、tokenizer、summarizer、model/NPU/Runtime；API 33 ARM64 debug probe 已通过 |
| P5 Memory consent HMI/API | `memory_consent_controller_defined=true`；`memory_consent_source_visibility_verified=true`；`memory_consent_disable_verified=true`；`memory_consent_preference_clear_verified=true`；`memory_consent_moving_restriction_verified=true`；`memory_consent_android13_arm64_verified=true`；`memory_consent_hmi_projection_only=true`；`memory_consent_repository_mutation_wired=false`；`memory_consent_production_authority_wired=false`；`memory_consent_runtime_wired=false`；`memory_consent_model_context_published=false`；`implementation_stage=P9-W03` | P5-W10 fixed-source API 与 debug 右侧半透明 HMI 完成；只更新 process-local projection，不是 production consent/repository erase；API 33 ARM64 debug probe 已通过 |
| P6 EventBroker interface/in-process | `event_broker_interface_defined=true`；`event_broker_typed_topics_verified=true`；`event_broker_append_before_notify_verified=true`；`event_broker_bounded_replay_filter_verified=true`；`event_broker_identity_policy_verified=true`；`event_broker_subscription_lifecycle_verified=true`；`event_broker_android13_arm64_verified=true`；`event_broker_process_local=true`；`event_broker_durable_persistence_wired=false`；`event_broker_dds_transport_wired=false`；`event_broker_production_published=false`；`event_broker_runtime_wired=false`；`implementation_stage=P9-W03` | P6-W01 pure-Java broker contract 完成；required 类名不代表 process-death durability，未接旧 Room cursor repository、DDS/SOME-IP 或 Runtime；API 33 ARM64 debug probe 已通过 |
| P6 Event Backpressure/QoS | `event_qos_contract_defined=true`；`event_qos_policy_count=4`；`event_qos_critical_no_silent_drop_verified=true`；`event_qos_deadline_priority_verified=true`；`event_qos_consumer_isolation_verified=true`；`event_qos_android13_arm64_verified=true`；`event_qos_process_local=true`；`event_qos_broker_wired=false`；`event_qos_durable_persistence_wired=false`；`event_qos_production_middleware_wired=false`；`implementation_stage=P9-W03` | P6-W02 完成四种显式压力策略和 critical no-silent-drop；独立 queue 尚未接 P6-W01 broker/durable repository/production middleware；API 33 ARM64 debug probe 已通过 |
| P6 TriggerRule/TriggerEngine | `trigger_rule_manifest_defined=true`；`trigger_rule_manifest_verified=true`；`trigger_threshold_window_debounce_verified=true`；`trigger_cooldown_scope_verified=true`；`trigger_input_fail_closed_verified=true`；`trigger_suggestion_only_verified=true`；`trigger_engine_android13_arm64_verified=true`；`trigger_engine_process_local=true`；`trigger_cooldown_persistence_wired=false`；`trigger_source_adapter_wired=false`；`trigger_auto_execution_enabled=false`；`trigger_runtime_wired=false`；`implementation_stage=P9-W03` | P6-W03 suggestion-only evaluator 完成；不接可信 source、durable cooldown、Runtime/Effect；API 33 ARM64 debug probe 已通过 |
| P6 Proactive consent/policy | `proactive_consent_policy_defined=true`；`proactive_grant_binding_verified=true`；`proactive_high_critical_generic_grant_blocked=true`；`proactive_grant_ttl_revoke_verified=true`；`proactive_policy_fail_closed_verified=true`；`proactive_consent_android13_arm64_verified=true`；`proactive_policy_process_local=true`；`proactive_grant_persistence_wired=false`；`proactive_consent_authority_wired=false`；`proactive_auto_execution_enabled=false`；`proactive_runtime_wired=false`；`implementation_stage=P9-W03` | P6-W04 policy-only exact grant 完成；HIGH/CRITICAL 必须显式审批，eligible 不授权 Effect；API 33 ARM64 debug probe 已通过 |
| P6 Context source adapters | `context_source_adapter_contract_defined=true`；`context_source_count=3`；`context_source_allowlist_verified=true`；`context_source_runtime_health_verified=true`；`context_source_simulated_vehicle_verified=true`；`context_source_time_verified=true`；`context_source_freshness_quality_verified=true`；`context_source_fail_closed_verified=true`；`context_source_android13_arm64_verified=true`；`context_source_production_registry_published=false`；`context_source_runtime_wired=false`；`context_source_trigger_engine_wired=false`；`vehicle_signal_provider_wired=false`；`vehicle_property_mapping_configured=false`；`implementation_stage=P9-W03` | P6-W05 三来源 normalization 完成；真实 vehicle source 后置 P8，API 33 ARM64 debug probe 已通过 |
| P6 Active suggestion UX | `active_suggestion_controller_defined=true`；`active_suggestion_full_card_verified=true`；`active_suggestion_merge_replay_verified=true`；`active_suggestion_moving_minimal_verified=true`；`active_suggestion_never_ask_verified=true`；`active_suggestion_android13_arm64_verified=true`；`active_suggestion_hmi_projection_only=true`；`active_suggestion_production_source_wired=false`；`active_suggestion_preference_repository_wired=false`；`active_suggestion_voice_engine_wired=false`；`trigger_engine_wired=false`；`graph_execution_enabled=false`；`effect_dispatch_enabled=false`；`implementation_stage=P9-W03` | P6-W06 why/cooldown/never-ask、merge 与 moving minimal projection 已完成；debug HMI 未接 production suggestion/Client2/Effect，API 33 ARM64 debug probe 已通过 |
| P6 Android aggregate probe acceptance | `p6_android13_arm64_probe_acceptance_complete=true`；`p6_probe_module_count=6`；`device_identity_redacted=true`；`production_event_middleware_published=false`；`production_trigger_runtime_wired=false`；`production_proactive_authority_published=false`；`production_context_source_registry_published=false`；`production_active_suggestion_source_wired=false`；`production_runtime_wired=false`；`driver_hal_accessed=false`；`hardware_accessed=false`；`production_ready=false`；`target_hardware_validated=false` | P6-W01..W06 Event/QoS/Trigger/Consent/Context/Suggestion 的 API 33 ARM64 debug probe 与完整安装回归通过；不构成量产事件中间件、主动授权或真实上下文源 |
...
| P7 ModelRequest/Result v2 | `model_contract_v2_defined=true`；`model_request_v2_fields_verified=true`；`model_result_v2_binding_verified=true`；`model_privacy_fallback_fail_closed=true`；`model_raw_content_accepted=false`；`model_provider_registry_wired=false`；`model_policy_router_wired=false`；`model_contract_v2_android13_arm64_verified=true`；`model_invoked=false`；`npu_accessed=false`；`implementation_stage=P9-W03` | P7-W01 digest-only request/result、预算/隐私约束和结果绑定已完成；不接 Provider registry/router、真实内容、模型/NPU/Effect，API 33 ARM64 debug probe 已通过 |
| P7 ModelProviderRegistry/health | `model_provider_registry_defined=true`；`model_provider_count=5`；`model_provider_health_freshness_verified=true`；`model_provider_health_replay_verified=true`；`model_provider_availability_separation_verified=true`；`model_provider_placeholder_fail_closed=true`；`model_contract_test_available_count=1`；`model_development_available_count=1`；`model_target_integration_available_count=0`；`model_production_ready_count=0`；`model_provider_registry_android13_arm64_verified=true`；`model_provider_registry_runtime_wired=false`；`model_policy_router_wired=false`；`implementation_stage=P7-R3-OC` | P7-W02 catalog/health contract 已扩展 OpenClaw target profile；无健康报告时 target availability 仍为 0，production 保持失败关闭 |
| P7 PolicyAwareModelRouter | `model_policy_router_defined=true`；`model_policy_router_privacy_network_thermal_verified=true`；`model_policy_router_latency_capability_quota_verified=true`；`model_policy_router_fallback_bounded=true`；`model_policy_router_no_action_authority=true`；`model_policy_router_android13_arm64_verified=true`；`model_policy_router_runtime_wired=false`；`provider_invoked=false`；`model_invoked=false`；`network_accessed=false`；`npu_accessed=false`；`implementation_stage=P9-W03` | P7-W03 deterministic admission 完成；只输出 primary/fallback/rejection metadata，不接 Provider/Runtime/Graph/Effect/network/NPU/hardware，API 33 ARM64 debug probe 已通过 |
| P7 LocalModelProvider | `local_model_provider_verified=true`；`local_model_provider_deadline_verified=true`；`local_model_provider_cancel_verified=true`；`local_model_provider_stream_limit_verified=true`；`local_model_provider_debug_only=true`；`local_model_provider_release_source_absent=true`；`local_model_provider_runtime_wired=false`；`local_model_provider_vendor_npu_fallback_enabled=false`；`local_model_provider_android13_arm64_verified=true`；`production_inference_enabled=false`；`implementation_stage=P9-W03` | P7-W04 debug-only 进程内 Provider 完成；支持 injected engine、deadline/cancel 与 bounded stream，不进入 release/生产路径，不接 Vendor NPU/Runtime/硬件，API 33 ARM64 debug probe 已通过 |
| P7 Structured Model Output | `structured_model_output_verified=true`；`model_output_catalog_binding_verified=true`；`model_output_unknown_capability_rejected=true`；`model_output_no_action_authority=true`；`model_output_schema_runtime_wired=false`；`structured_model_output_android13_arm64_verified=true`；`model_invoked=false`；`raw_model_content_logged=false`；`implementation_stage=P9-W03` | P7-W05 strict scenario/parameter/summary schema 完成；ScenarioManifest + CapabilityCatalog 约束 type/range/area，proposal 不授予 Plan/approval/Effect authority，API 33 ARM64 debug probe 已通过 |
| P7 Scenario Evaluation | `scenario_evaluation_verified=true`；`evaluation_corpus_verified=true`；`evaluation_metrics_verified=true`；`evaluation_boundary_verified=true`；`evaluation_case_count=12`；`scenario_evaluation_runtime_wired=false`；`raw_evaluation_content_logged=false`；`scenario_evaluation_android13_arm64_verified=true`；`model_invoked=false`；`production_ready=false`；`implementation_stage=P9-W03` | P7-W06 fixed synthetic evaluator 完成；只保留 digest/enums/counts，1000/0 deterministic probe 不代表真实模型质量或硬件资格，API 33 ARM64 debug probe 已通过 |
| P7 Resource Admission | `model_resource_admission_verified=true`；`foreground_vehicle_priority_verified=true`；`thermal_degradation_verified=true`；`thermal_resource_fail_closed_verified=true`；`admission_boundary_verified=true`；`resource_admission_runtime_wired=false`；`resource_snapshot_producer_wired=false`；`model_resource_admission_android13_arm64_verified=true`；`provider_invoked=false`；`model_invoked=false`；`hardware_accessed=false`；`production_ready=false`；`implementation_stage=P9-W03` | P7-W07 request/route/policy/resource binding、foreground HIGH、compact/minimal degradation 与 scheduler composition 已完成；真实 producer、Provider/NPU 仍未接，API 33 ARM64 debug probe 已通过 |
| P7 Android aggregate probe acceptance | `p7_android13_arm64_probe_acceptance_complete=true`；`p7_probe_module_count=7`；`device_identity_redacted=true`；`production_model_provider_published=false`；`production_model_router_wired=false`；`production_inference_enabled=false`；`production_model_output_runtime_wired=false`；`production_evaluation_authority_published=false`；`production_resource_snapshot_provider_wired=false`；`production_runtime_wired=false`；`provider_invoked=false`；`model_invoked=false`；`network_accessed=false`；`npu_accessed=false`；`driver_hal_accessed=false`；`hardware_accessed=false`；`production_ready=false`；`target_hardware_validated=false` | P7-W01..W07 模型合同/Registry/Router/Provider/Output/Evaluation/Resource 的 API 33 ARM64 debug probe 与完整安装回归通过；不构成真实模型、NPU 或模型质量验收 |
| P9 Android aggregate debug probe acceptance | `p9_android13_arm64_probe_acceptance_complete=true`；`p9_probe_module_count=7`；`device_identity_redacted=true`；`performance_budget_contract_probe_android13_arm64_verified=true`；`stability_matrix_contract_probe_android13_arm64_verified=true`；`security_boundary_probe_android13_arm64_verified=true`；`privacy_redaction_probe_android13_arm64_verified=true`；`release_metadata_probe_android13_arm64_verified=true`；`driver_safety_android_contract_probe_android13_arm64_verified=true`；`field_diagnostics_probe_android13_arm64_verified=true`；`performance_budget_target_measurement_complete=false`；`stability_target_72h_complete=false`；`security_coverage_guided_fuzz_complete=false`；`privacy_owner_policy_approved=false`；`production_signer_owner_approved=false`；`driver_safety_android13_arm64_verified=false`；`field_diagnostics_target_category_execution_complete=false`；`release_evidence_target_report_admitted=false`；`driver_hal_accessed=false`；`hardware_accessed=false`；`production_ready=false`；`target_hardware_validated=false` | P9 七个 hardening/release debug application probes 与完整安装回归通过；不构成 target performance、72h、fuzz、owner policy、正式 signer/rollback、OEM vehicle safety 或 target retest |
| 中控 AIOS 演示闭环 | `cockpit_demo_control_loop_implemented=false`；`hmi_d4_debug_demo_control_loop_complete=true`；`hmi_d4_demo_control_loop_complete=false` | P4-D4e Client2 debug 仿真闭环已完成；真实 Runtime Plan/Effect、production 审批/撤销、车辆回读与 adapter 尚未形成量产闭环 |
...
flowchart TB
  subgraph App["应用与 HMI"]
    ...
    VoiceFirstUi["两个自然场景触发（P4-R3）"]
    LiveTraceUi["32 行实时调用链（P4-R3）"]
    ActuatorFeedbackUi["HVAC / Seat SIMULATED 动画（P4-R3）"]
    ClientBridge["Client2 Session/Event bridge（P4-W01 已完成）"]
    HmiState["Immutable HMI state / reducer（工程回归保留）"]
    IntentUi["历史四阶段/设备面板（P4-R3 不暴露）"]
    PlanUi["计划与 Policy 工程状态"]
    ExecutionUi["Effect 工程状态"]
    ResultUi["Readback 工程状态"]
    DeviceUi["HVAC / Seat 手动面板（P4-R3 不暴露）"]
    ...
    ClientOrchestration["OrchestrationClient V1（P4-R2）"]
  end
  ...
  subgraph Binder["Android Protocol Binding"]
    ...
    OrchestrationApi["ICentralBrainOrchestration V1（P4-R1）"]
    DevModelProjectionApi["ICentralBrainDevelopmentModelProjection V1（SDK debug-only）"]
  ...
  subgraph Runtime["Android AIOS Runtime"]
    ...
    ToolExecutor["Signed built-in in-process executor（P5-W04）"]
    OrchestrationRuntime["Owner/session Orchestration + Room projection（P4-R1）"]
    RuntimeComposition["Debug Tool/Skill/Context/WorkingMemory composition（P5-R1）"]
    DecisionComposition["Debug Context/Trigger/Consent/Model/Event composition（P6-P7-R1）"]
    CockpitPrompt["Automotive cockpit prompt + required actions（P4-R3）"]
    OllamaGateway["Fixed Ollama Model Gateway（P7-R2 debug verified）"]
    OpenClawGateway["Fixed OpenClaw WebSocket v3 Gateway（P7-R3-OC2 target profile）"]
    DevModelProjectionStore["Owner-scoped model reply store（debug-only / non-durable）"]
  ...
  subgraph DevelopmentCompute["开发模型仿真"]
    AdbReverse["ADB reverse tcp:11434"]
    WslOllama["WSL Ollama / qwen3.5:27b-optimized"]
    TargetOpenClaw["Target OpenClaw 169.254.208.110:18789（transitional）"]
  end

  subgraph Target["目标硬件外部边界"]
    Vehicle["AAOS/Vendor vehicle service"]
    VendorNpu["Vendor NPU runtime / PCIe card"]
    ProductionOllama["Ollama compute base 169.254.208.110（P7-R3 planned）"]
  ...
  Client2 --> VoiceFirstUi --> ClientBridge
  ClientBridge -- "owner Session ID" --> ClientOrchestration --> OrchestrationApi
  OrchestrationApi -- "Plan/Graph/Effect/readback projection" --> HmiState
  DevModelProjectionApi -- "validated reply / provider / latency" --> ClientOrchestration
  ClientOrchestration -- "bounded pipeline milestones" --> LiveTraceUi
  HmiState -- "SIMULATED Effect state" --> ActuatorFeedbackUi
  ...
  EventApi --> Services
  OrchestrationApi --> OrchestrationRuntime
  OrchestrationRuntime --> RuntimeComposition
  ...
  ToolExecutor -->|"debug metadata-only P5-R1"| RuntimeComposition
  SkillVerifier -->|"compiled-in governance admission"| RuntimeComposition
  ContextBudget -->|"metadata decision"| RuntimeComposition
  RuntimeComposition -->|"digest-only session state"| WorkingMemory
  RuntimeComposition -. "production composition pending" .-> Domains
  ContextSourcesP6 -->|"debug normalized digest"| DecisionComposition
  TriggerEngineP6 -->|"suggestion-only"| DecisionComposition
  ProactiveConsentP6 -->|"no-grant fail closed"| DecisionComposition
  ModelRouterP7 -->|"WSL Ollama; deterministic probe retained"| DecisionComposition
  EventBrokerP6 -->|"two digest events"| DecisionComposition
  RuntimeComposition -->|"combined Node/Effect evidence"| DecisionComposition
  CockpitPrompt --> DecisionComposition
  DecisionComposition --> OllamaGateway --> AdbReverse --> WslOllama
  DecisionComposition --> OpenClawGateway --> TargetOpenClaw
  DecisionComposition --> DevModelProjectionStore --> DevModelProjectionApi
  Domains --> Twin
  Twin --> Context
  ...
  OllamaGateway -. "release Provider disabled" .-> ProductionOllama --> VendorNpu
  OpenClawGateway -. "debug target integration; no direct NPU claim" .-> VendorNpu
...
| P4-D4a Simulated Scenario Graph | `simulated_scenario_graph_defined=true`；debug-only Compiler/AgentGraph composition；Context/Policy/Summary auto projection；Approval/Effect/Readback pending node | 八组 JVM 已验证；未接 Android Runtime/Client2/adapter/readback，`scenario_execution_enabled=false` | `DEVELOPED` |
| P4-D4b Simulated Scenario Runtime | `simulated_scenario_debug_runtime_projection_defined=true`；进程内 Session/Plan/Event projection；复用 `BoundedEventRuntime` 两类 topic 与八类 schema | 八组 JVM 已验证；未发布 Android Service/Binder，未接 Client2/adapter/readback | `DEVELOPED` |
| P4-D4c Simulated Scenario Binder | debug-only AIDL v1、Parcelable metadata、固定 Cold/Fatigue x Parked/Moving 输入、签名权限与 capability 双门禁 | 六组 JVM + Debug/Release 构建；Android 13 安装可见且未授权调用被拒 | `DEVELOPED` |
| P4-D4d Simulated Effect Composition | Binder/Parcelable v2、HVAC/Seat/Media/Nav simulated adapter 自动 dispatch、MATCHED readback、显式 approval、Partial/Stuck 投影 | 八组 JVM；实体 Android 13 同签名 probe 通过 8 dispatch/6 matched readback/1 approval/0 failure；只执行模拟目标，未接 Client2/真实 Vehicle/NPU/Driver-HAL | `DEVELOPED` |
| P4-D4e Client2 Simulated Scenario Chain | Client2 Binder v2 exact-wire DTO、reducer-owned 七阶段链路、显式 debug approval/skip、固定失败关闭映射 | Host reducer + APK 构建；实体 Android 13 ARM64 通过 Cold 3/3、Fatigue approved 5/3、rejected Partial 4/2 与两次显式输入 | `DEVELOPED / DEBUG VERIFIED` |
| P4-R2 Client2 Orchestration V1 migration | `SessionClient -> SessionHandle -> OrchestrationClient V1 -> typed Plan validation -> reducer`；projection-bound approval；reducer-owned 七阶段执行 timeline；删除旧场景 Binder/Parcelable | SDK/APK/static 与 Android 13 x86_64 Cold/Fatigue approve/reject 全链通过；ARM64、production/vehicle/hardware 不提升 | `DEVELOPED / API33 X86 VERIFIED` |
| P5 Android aggregate probe acceptance | `p5_android13_arm64_probe_acceptance_complete=true`；`p5_probe_module_count=10`；`tool_manifest_android13_arm64_verified=true`；`tool_registry_android13_arm64_verified=true`；`tool_rule_solver_android13_arm64_verified=true`；`tool_executor_android13_arm64_verified=true`；`skill_package_verifier_android13_arm64_verified=true`；`working_memory_android13_arm64_verified=true`；`profile_memory_android13_arm64_verified=true`；`episodic_memory_android13_arm64_verified=true`；`context_budget_android13_arm64_verified=true`；`memory_consent_android13_arm64_verified=true` | API 33 / ARM64 统一安装回归通过；`device_identity_redacted=true`；`production_tool_authority_published=false`；`production_memory_authority_published=false`；`production_runtime_wired=false`；`driver_hal_accessed=false`；`hardware_accessed=false`；`production_ready=false`；`target_hardware_validated=false` | `DEVELOPED / DEBUG PROBE VERIFIED` |
| Tool Manifest/Schema | versioned identity/owner、bounded scalar input/output、capability/risk/timeout/idempotency/health、canonical digest、exact validator | JVM + debug/release compile + Android 13 ARM64 debug probe；不接 production Registry/Executor | `DEVELOPED / DEBUG VERIFIED` |
| Tool Registry/Resolver | immutable sorted registry、same-version digest conflict、highest-compatible resolution、dynamic health freshness、registered/resolved/usable 分层 | JVM + debug/release compile + Android 13 ARM64 debug probe；不接 Runtime/Graph/Executor | `DEVELOPED / DEBUG VERIFIED` |
| Tool RuleSolver | six-rule immutable graph、canonical digest、tri-state condition、rule/model/USABLE intersection、terminal prerequisite、approval no-grant | JVM + debug/release compile + Android 13 ARM64 debug probe；不接 Runtime/Graph/Executor | `DEVELOPED / DEBUG VERIFIED` |
...
| P5 WorkingMemoryStore | owner/session 隔离、monotonic TTL、item/byte/token budgets、防御性 payload copy、terminal tombstone 与 retained-byte cleanup | JVM + debug/release compile + Android 13 ARM64 debug probe；process-local only，未接 Runtime/Room/Graph/model | `DEVELOPED / DEBUG VERIFIED` |
| P5 ProfileMemoryStore | explicit consent、fixed typed field allowlist、user/seat scope、read/update/delete/export、encryption-owner gate、sealed-byte cleanup | JVM + debug/release compile + Android 13 ARM64 debug probe；production consent/Keystore/Room/Runtime 未接 | `DEVELOPED / DEBUG VERIFIED` |
| P5 EpisodicMemoryStore | build-owned scenario ref、typed trigger/result/outcome、owner isolation、policy/read/erase gate、replay/conflict、retention/capacity | JVM + debug/release compile + Android 13 ARM64 debug probe；production authority/repository/Runtime 未接 | `DEVELOPED / DEBUG VERIFIED` |
| P5 ContextBudgetManager | SYSTEM/CONTEXT/PROFILE/EPISODE/HISTORY 固定顺序、category/global token+byte 包络、required fail-closed、deterministic summarize/truncate/drop 指令 | JVM + debug/release compile + Android 13 ARM64 debug probe；metadata/decision-only，不接 tokenizer/summarizer/model/NPU/Runtime | `DEVELOPED / DEBUG VERIFIED` |
| P5 Memory consent HMI/API | 固定来源/purpose/retention、retained switch、preference clear、owner evidence、replay/conflict、PARKED-only management、响应式 debug HMI | JVM + debug/release compile + Android 13 ARM64 debug probe；process-local projection only，production authority/repository/Runtime 未接 | `DEVELOPED / DEBUG VERIFIED` |
| P6 EventBroker interface/in-process | 三个 generic typed topic、per-topic cursor、append-before-notify、bounded replay/filter、identity/policy、owner subscription | JVM + debug/release compile；process-local only，未接 Room cursor repository、DDS/SOME-IP、Binder/Runtime，API 33 ARM64 debug probe 已通过 | `DEVELOPED` |
...
| P6 TriggerRule/TriggerEngine | bounded canonical manifest、typed threshold/window/debounce、rule+scope atomic cooldown、input fail-closed、digest-only ScenarioSuggestion | JVM + debug/release compile；process-local suggestion-only，未接 source adapter、durable cooldown、consent/policy、Runtime/Effect，API 33 ARM64 debug probe 已通过 | `DEVELOPED` |
| P6 Proactive consent/policy | PARKED evidence/authority grant mutation、owner/scenario/capability/zone/risk/TTL exact binding、HIGH/CRITICAL hard block、revoke/replay/conflict | JVM + debug/release compile；process-local policy-only，eligible 不授权 Effect，未接 production authority/persistence/Runtime，API 33 ARM64 debug probe 已通过 | `DEVELOPED` |
| P6 Context source adapters | fixed Runtime health/SIMULATED vehicle/time descriptor、typed observation、freshness/quality/provenance normalization、digest evidence | JVM + debug/release compile；normalization-only，未接 production registry/Trigger/Runtime/真实 vehicle，API 33 ARM64 debug probe 已通过 | `DEVELOPED` |
| P6 Active suggestion UX | fixed reason/why/plan/voice key、owner+scenario+zone merge、replay/conflict、dismiss cooldown、PARKED-only never-ask、moving/unknown minimal banner | 六项 JVM + debug/release compile；debug projection-only，未接 production source/Client2/voice/Graph/Effect，API 33 ARM64 debug probe 已通过 | `DEVELOPED` |
| P7 ModelRequest/Result v2 | purpose/privacy/capability/fallback fixed enum、latency/token hard bound、trace/input digest、canonical fingerprint、result request binding | 六项 JVM + debug/release compile；contract-only，未接 Provider registry/router、raw content、model/NPU/Effect，API 33 ARM64 debug probe 已通过 | `DEVELOPED` |
| P7 ModelProviderRegistry/health | fixed five-provider catalog、capability/source binding、revision/replay/conflict、monotonic freshness、test/dev/target/production separation | JVM + debug/release compile；OpenClaw target profile 可用于受控 debug 集成，production eligibility 仍为 false | `DEVELOPED` |
...
| P7-R2 Ollama development gateway | 固定 dev/prod endpoint、debug HTTP engine、schema/action allowlist、ADB reverse、独立 debug-only owner-scoped Binder reply | 网关 API 33 ARM64 实际 WSL 模型调用通过；最终投影 host/build 通过、ARM64 复测因 ADB 0 设备待执行；Orchestration V1 未变，release Service/量产 Provider/NPU 保持 false | `SOFTWARE COMPLETE / GATEWAY HW VERIFIED` |
| P7-R3-OC2 OpenClaw target gateway | 固定 target WebSocket v3、源码/APK 固定 token、challenge/auth、send/abort/history、严格输出、target Router profile、Client2 投影 | 历史 API 33 ARM64 Runtime/Client2 真实 OpenClaw 证据保留；当前 18789 服务未监听，固定凭据和 cleartext/release qualification 偏差未关闭 | `TRANSITIONAL TARGET INTEGRATION / RETEST BLOCKED` |
...
| P8 Target Capability Discovery 软件准备与公开清单 | versioned 14-column/8-capability contract、read-only redacted collector、API 33 public inventory aggregate、private raw evidence boundary | 公开清单已脱敏采集；未取得 OEM property/service/permission evidence，capability mapping 仍为 `EXTERNAL_BLOCKED` | `DEVELOPED` |
...
| P9 Security Boundary Inventory | `security_aidl_parcel_inventory_complete=true`；`security_aidl_surface_count=37`；7 interface、30 parcelable；`security_validation_family_count=8`；`security_host_path_oversize_aggregate_verified=true`；`security_android_debug_probe_available=true`；`security_android_debug_probe_executed=true`；`security_boundary_probe_android13_arm64_verified=true` | W03c host/JVM 与 API 33 ARM64 aggregate debug probe 已验证；coverage-guided fuzz 仍未完成，Binder UID/current signer evidence 由独立 W03d 承担 | `DEVELOPED / DEBUG VERIFIED` |
| P9 Binder Identity Device Evidence | `security_identity_device_probe_verified=true`；`security_distinct_app_uids_verified=true`；`security_binder_calling_uid_spoof_android_verified=true`；`security_package_signature_cryptographically_verified=true`；`security_same_signer_debug_binding_verified=true`；`security_production_signer_verified=false` | W03d 类型化 debug AIDL 已在 API 33 ARM64 跨进程验证；只证明 Binder UID 与已安装 debug APK 当前 signer 获取链，coverage fuzz、Runtime wiring 与目标硬件资格仍为 false | `DEVELOPED / DEBUG VERIFIED` |
| P9 Callback Replay Device Evidence | task ID/strict sequence/single-terminal guard、same-owner active/terminal replay、conflict silence、cross-UID owner isolation | W03e 已在 API 33 ARM64 真实 task Binder 验证；debug test principal 不进入 release，production signer/owner 仍未完成 | `DEVELOPED / DEBUG VERIFIED` |
| P9 External Security Evidence Interface | release/source/archive/device alias/profile/result digest/internal reference/privacy confirmation 八字段非执行型接口 | W03g 仅保留外部证据接收边界；仓库执行器、Android/native component 和自动执行均不存在 | `INTERFACE ONLY / SUSPENDED` |
...
| P9 Driver Safety Redacted Probe | `driver_safety_redacted_projection_defined=true`；27 个 count/boolean key；DUMP debug Activity；只读 no-install ADB adapter | W06b host/JVM/debug-release 软件边界已验证；目标 probe 未执行；不读取 vehicle scalar/owner reference/raw log，不构成 OEM safety evidence | `DEVELOPED` |
| P9 Release Evidence Envelope | `release_evidence_envelope_defined=true`；8 类有序 diagnostics；strict release identity；stable report digest | W07a host/JVM 已验证；不采集目标、不自动上传；target owner/report/Runtime/retest/hardware claims 均为 false | `DEVELOPED` |
| P9 Field Diagnostics Probe | `field_diagnostics_projection_defined=true`；31 个 count/boolean key；DUMP debug Activity；5 executed + 3 NOT_RUN adapter | W07b host/JVM/debug-release 软件边界已验证；目标 probe 未执行；不安装/上传或输出 identity/signer/raw log/payload | `DEVELOPED` |
| P9 Release Retest Workflow | `release_retest_state_machine_defined=true`；5 state/5 transition；strict replacement identity；four-party digest admission | W07c host/JVM 软件边界已验证；不发布/安装 release、不修改/自动关闭 Issue；target owner/tester evidence 外部阻塞 | `DEVELOPED` |
...
`model_contract_v2_android13_arm64_verified=true`；`model_invoked=false`；
`model_provider_registry_defined=true`；`model_provider_count=5`；
...
`local_model_provider_android13_arm64_verified=true`；`production_inference_enabled=false`；
...
| 模块 | 最小剩余工作 | 阻塞或下一步 | 状态 |
| --- | --- | --- | --- |
| 场景解析与量产编排 | catalog/resolver/compiler、P4-R1 durable Orchestration 与 P4-R2 Client2 formal SDK debug 闭环均已完成 | production Scenario/Effect authority、真实 adapter 与发布 owner 见 `ISSUE-022/030` | `SOFTWARE_COMPLETE / EXTERNAL_BLOCKED` |
| Event Broker production/durability | Event V1/V2、Room cursor/ACK、process broker/QoS 和 debug decision composition 软件完成 | production durable publisher/middleware/callback owner 见 `ISSUE-046` | `SOFTWARE_COMPLETE / EXTERNAL_BLOCKED` |
| Proactive consent production | Trigger、exact consent、HIGH/CRITICAL hard block、suggestion projection 与 debug composition 软件完成 | 真实 identity/receipt、durable revoke、产品/隐私 owner 见 `ISSUE-031` | `SOFTWARE_COMPLETE / EXTERNAL_BLOCKED` |
| Context source production | typed Context、freshness/trust、SIMULATED normalization 与 debug decision composition 软件完成 | 真实 vehicle/DMS/time producer 与 source owner 见 `ISSUE-030/031` | `SOFTWARE_COMPLETE / EXTERNAL_BLOCKED` |
| Active suggestion production | policy/cooldown/DND projection、debug HMI 与 decision chain 软件完成 | production event/source/voice/product owner 见 `ISSUE-031` | `SOFTWARE_COMPLETE / EXTERNAL_BLOCKED` |
| P7-R3 Model Runtime production | P7-R3-OC2 已完成当前 OpenClaw 过渡 target profile、固定凭据和 Client2 投影；历史真机调用证据保留，当前端口监听恢复后需重测；仍需正式 credential/TLS、health/version、artifact owner、release Provider 与 Ollama 迁移 | 目标服务监听、NPU 直接证据、可信 resource producer、链路安全和标定 owner 见 `ISSUE-024/044/054` | `TRANSITIONAL INTEGRATION / EXTERNAL_VALIDATION` |
| P8-W01 目标能力发现 | 软件合同、只读采集器和 API 33 脱敏公开清单已完成；仍缺 property/service/area/type/read-write/permission/owner/version 的目标证据 | `DEV-085/110`、`ISSUE-047`；OEM/Vendor 输入未提供 | `EXTERNAL_BLOCKED` |
| P9-W03 Security review | W03a..W03e 的确定性边界、identity 和 callback replay 证据保留；W03f 可执行 campaign 已按用户决策撤回 | W03g 只保留外部证据接口；不申请可信访问权限，不在仓库执行该类测试；见 `DEV-114`、`ISSUE-050` | `SUSPENDED / EXTERNAL_BLOCKED` |
...
| P9-W06 Driver safety | W06a/W06b 已完成失败关闭准入合同、27-key 脱敏 projection、debug Activity 和只读 adapter | OEM owner、真实 vehicle state、驾驶分心矩阵和座椅硬联锁验收仍由 `DEV-096/097`、`ISSUE-029/030` 外部阻塞 | `EXTERNAL_BLOCKED` |
| P9-W07 Release evidence/diagnostics | W07a envelope、W07b 31-key probe/adapter、W07c 5-state/5-transition release-retest workflow 已完成 | 命名 replacement release、完整 target report 与 owner/tester 复测见 `DEV-098/099/100`、`ISSUE-052/053` | `EXTERNAL_BLOCKED` |
| Durable Agent Graph production wiring | P3-W01..W09 + P4-R1/P4-R2 Runtime/Binder/Room/Client2 软件 composition 完成 | 真实 Effect material/authority、retention/encryption 与 target fault evidence 见 `ISSUE-022` | `SOFTWARE_COMPLETE / EXTERNAL_BLOCKED` |
| 中控 AIOS 量产闭环 | 自然输入、formal Plan/Graph、自动 simulated Effect/readback、显式审批和七阶段执行 timeline 已完成 | 真实车辆执行、production approval/undo authority 与 target validation 见 `ISSUE-019/023/030/033` | `SOFTWARE_COMPLETE / EXTERNAL_BLOCKED` |
| Tool/Skill/Memory runtime | P5-W01..W10 与 P5-R1 Tool/Skill/Memory/Context/consent debug composition 软件完成 | production signer、storage、identity、privacy owner 见 `ISSUE-040..045` | `SOFTWARE_COMPLETE / EXTERNAL_BLOCKED` |
| 量产 HMI 加固 | 1920x1080 safe-frame、display/accessibility、driving restriction、recovery 与 debug 闭环软件基线完成 | OEM 可维护源码、Car UX/分心矩阵、长稳性能、production signer 见 `ISSUE-019/048/049` | `SOFTWARE_BASELINE_COMPLETE / EXTERNAL_BLOCKED` |
...
| `apk-labs/client2-central-brain/` | 座舱演示 HMI | `Client2ScenarioBridge`、`OrchestrationRuntimeClient`、`CockpitDisplayPolicy`、immutable HMI/device/execution/recovery/engineer state、唯一 reducer、四阶段 overlay、HVAC/Seat controls、七阶段 execution projection、projection-bound debug approval、protected engineer drawer 与 APK patch/build/test；旧 `ISimulatedScenarioRuntime` Client2 链已删除，production execution 仍未发布 |
...
bash tools/probe_central_brain_android_field_diagnostics.sh
```
...
P9-W06b 证据键：`driver_safety_redacted_projection_defined=true`、`driver_safety_audit_key_count=27`、
`driver_safety_android_debug_probe_available=true`、`driver_safety_android_debug_probe_executed=false`、
`driver_safety_target_adapter_defined=true`、`driver_safety_android13_arm64_verified=false`。

P9-W07a 证据键：`release_evidence_envelope_defined=true`、`release_evidence_diagnostic_category_count=8`、
...
P9-W07b 证据键：`field_diagnostics_projection_defined=true`、`field_diagnostics_audit_key_count=31`、
...
| [发布证据与现场诊断](docs/CENTRAL_BRAIN_RELEASE_EVIDENCE_FIELD_DIAGNOSTICS.md) | P9-W07 release identity、八类 diagnostics、report digest 与 target/retest 边界 |
...
| 2026-07-19 | [P7-R2 WSL Ollama development gateway](central-brain/contracts/central_brain_android_ollama_gateway_v1.json) | 新增固定开发/量产端点、debug HTTP provider、schema/action 校验、ADB reverse 工具和独立 debug-only 不落库 Binder 回复投影 | API 33 ARM64 实际模型调用成功；最终投影 host/build 通过、ARM64 复测因 ADB 0 设备待执行；Orchestration V1 未变，release Service/量产 Provider/NPU/车辆 Effect仍为 false |
| 2026-07-19 | [P10-R1 Android repository software completion](central-brain/contracts/central_brain_android_software_completion_v1.json) | 清理历史待开发状态；所有剩余 Req ID 归类为仓库软件完成、外部阻塞、挂起或范围外；新增完成度聚合门禁 | 仓库未分类需求为 0；production/target 仍 false；security campaign 保持挂起 |
| 2026-07-19 | [P4-R2 Client2 Orchestration V1 migration](central-brain/contracts/central_brain_android_client2_orchestration_migration_v1.json) | Client2 改用正式 Orchestration SDK；修复 recovery key、Session/Plan deadline、稳定 approval projection 与 capability-bound readback；删除旧场景 Binder client/Parcelable | SDK/APK/static 与 API 33 x86_64 Cold 3/3、Fatigue approve 5/3、reject 4/2 通过；ARM64/production Context/vehicle/NPU/Driver-HAL 与 target qualification 仍为 false |
| 2026-07-19 | [P5-R1 Runtime composition](central-brain/contracts/central_brain_android_runtime_composition_v1.json) | debug Orchestration 串联 Tool/Skill/ContextBudget/WorkingMemory，节点/Effect 绑定组合摘要并在终态清理 | JVM/compile 与 API 33 x86_64 子项通过；ARM64=false；release、Profile/Episodic、真实模型/车辆/NPU 均保持 false |
| 2026-07-19 | [P6-EV2 durable Session Event](central-brain/contracts/central_brain_android_event_v2.json) | 新增独立 Event V2 AIDL、终页 opaque cursor、Room 持久单调 ACK、owner/session 隔离、SDK V2 协商/V1 回退和 DUMP 数据库探针 | 仓库实现与 JVM 验证完成；冻结 V1 未改；Android 13 ARM64 物理验证待执行，production Event middleware/Vehicle/NPU 仍为 false |
| 2026-07-19 | [P9-W03g Security evidence interface](central-brain/contracts/central_brain_android_p9_security_evidence_interface.json) | 按用户决策删除 W03f 引擎依赖、Gradle task、Java target/test、seed 和 runner；只保留非执行型外部证据字段与禁止输入边界 | `security_requirement_suspended=true`；不申请可信访问权限；implementation/execution/admission/production/target 均为 false |
| 2026-07-19 | P9-W03f historical campaign | 曾交付短预算 host parser campaign，随后因平台内容分类和用户范围决策完整撤回可执行代码 | 历史记录，不是当前仓库能力；由 W03g interface-only 状态取代 |
| 2026-07-19 | [P9-W03e Callback replay device evidence](docs/CENTRAL_BRAIN_SECURITY_REVIEW_FUZZ.md) | SDK 增加 task ID/sequence/terminal 回调重放 guard；debug-only capability overlay 允许两个不同 UID 测试主体通过真实任务 Binder 验证 active replay、conflict、terminal replay 与跨 owner 隔离 | API 33 ARM64 通过，`security_task_callback_replay_android_verified=true`；coverage fuzz、production signer、安全 owner、Vehicle/NPU/Driver-HAL 与目标硬件资格仍为 false |
| 2026-07-19 | [P9-W03d Binder identity device evidence](docs/CENTRAL_BRAIN_SECURITY_REVIEW_FUZZ.md) | 新增同源 debug-only AIDL、signature-protected Runtime probe、SDK instrumentation 与 API 33 ARM64 测试；服务端使用 `Binder.getCallingUid()` 和 `PackageManager.GET_SIGNING_CERTIFICATES`，客户端独立计算当前 signer SHA-256 | 跨进程 UID/package/signer spoof 证据通过且不回传 raw identity；production signer、coverage fuzz、Runtime wiring、Vehicle/NPU/Driver-HAL 与目标硬件资格仍为 false |
| 2026-07-19 | [P1-W05 SDK facade lifecycle hardening](central-brain/android-runtime/central-brain-sdk/src/main/java/com/centralbrain/sdk/AndroidScenarioTransport.java) | 修复健康 `reconnect()` 仅清空客户端 callback 映射、未注销旧 Event Binder callback 的生命周期缺口；实机用例扩展为连续 6 次 reconnect + replay + cancel + process-death recovery | API 33 ARM64 通过，`healthy_reconnect_callback_cleanup_verified=true`；Scenario/Effect/Vehicle/NPU/Driver-HAL 仍关闭 |
| 2026-07-18 | [P9 Android aggregate debug probe acceptance](central-brain/contracts/central_brain_android_p9_physical_acceptance.json) | 固化 P9-W01/W02/W03c/W04c/W05b/W06b/W07b 的 API 33 ARM64 应用探针验收，并用 probe-specific marker 区分执行证据和生产资格 | 完整安装回归通过；target performance/72h/fuzz/privacy owner/production signer/OEM safety/target retest 与 hardware qualification 仍为 false |
| 2026-07-18 | [P7 Android aggregate probe acceptance](central-brain/contracts/central_brain_android_p7_physical_acceptance.json) | 固化 P7-W01..W07 model contract/registry/router/provider/output/evaluation/resource 的 API 33 ARM64 实机验收，并补齐 LocalModelProvider 派生 marker | 完整安装回归通过；debug application probe only，真实模型/NPU/network/Runtime/model quality 与 target qualification 仍为 false |
| 2026-07-18 | [P6 Android aggregate probe acceptance](central-brain/contracts/central_brain_android_p6_physical_acceptance.json) | 固化 P6-W01..W06 Event/QoS/Trigger/Consent/Context/Suggestion 的 API 33 ARM64 实机验收和六模块聚合门禁 | 完整安装回归通过；debug application probe only，production Event/Trigger/Consent/Context/Suggestion、Vehicle/NPU/Driver-HAL 与 target qualification 仍为 false |
| 2026-07-18 | [P5 Android aggregate probe acceptance](central-brain/contracts/central_brain_android_p5_physical_acceptance.json) | 固化 P5-W01..W10 十个 Tool/Skill/Memory debug probe 的 API 33 ARM64 实机验收，并修复 ContextBudget overflow fixture 与 installer 日志隔离/设备身份脱敏 | 完整安装回归通过；应用层 probe only，production Tool/Memory authority、Runtime、Vehicle/NPU/Driver-HAL 与 target qualification 仍为 false |
| 2026-07-18 | [P4-D4e Client2 Simulated Scenario Chain](docs/CENTRAL_BRAIN_AIOS_STAGE2_DEVELOPMENT_BACKLOG.md) | Client2 接入 Binder v2，显示七阶段自动执行链与显式审批；实体 Android 13 验证 Cold 3/3、Fatigue approved 5/3、rejected Partial 4/2 | Debug process-local simulation only；真实 Vehicle/NPU/Driver-HAL/production approval 未接，P8/P9 外部阻塞 |
| 2026-07-18 | [P4-D4d Simulated Effect Composition](docs/CENTRAL_BRAIN_AIOS_STAGE2_DEVELOPMENT_BACKLOG.md) | Binder/Parcelable v2，自动模拟 HVAC/Seat/Media/Nav Effect 与 MATCHED readback；实体 Android 13 同签名 probe 通过 8/6/1/0 聚合计数 | Debug simulation only；Client2/真实 Vehicle/NPU/Driver-HAL/production 未接；下一步 P4-D4e |
| 2026-07-18 | [P4-D4c Simulated Scenario Binder](docs/CENTRAL_BRAIN_AIOS_STAGE2_DEVELOPMENT_BACKLOG.md) | 新增 AIDL v1、Parcelable、固定场景输入工厂和签名/capability 双保护 debug Service | Android 13 安装成功，服务/动作/权限可见，ADB shell 未授权调用被拒；同签名正向 Binder、Client2/adapter/readback/production 均未完成；下一步 P4-D4d |
| 2026-07-18 | [P4-D4b Simulated Scenario Runtime](docs/CENTRAL_BRAIN_AIOS_STAGE2_DEVELOPMENT_BACKLOG.md) | 新增 debug-only 进程内 Session/Plan/Event projection、两类可信 topic、八类 schema 和八组 JVM tests | 不发布 Android Service/Binder，不接 Client2/adapter/readback；下一步 P4-D4c |
| 2026-07-18 | [P4-D4a Simulated Scenario Graph](docs/CENTRAL_BRAIN_AIOS_STAGE2_DEVELOPMENT_BACKLOG.md) | 新增 debug-only Compiler/AgentGraph composition、Context/Policy 自动推进、Approval/Effect/Readback pending node 和八组 JVM tests | 不接 Runtime/Client2/adapter；Effect/readback/hardware/production 均 false；下一步 P4-D4b |
| 2026-07-18 | [P9-W07c Release Retest Workflow](docs/CENTRAL_BRAIN_RELEASE_EVIDENCE_FIELD_DIAGNOSTICS.md) | 新增 5-state/5-transition workflow、strict replacement identity、四方 digest admission、PASS verified 与非 PASS fix-ready 循环 | pure Java；不发布/安装/关单；真实 target/owner/tester evidence 外部阻塞 |
| 2026-07-18 | [P9-W07b Field Diagnostics Probe](docs/CENTRAL_BRAIN_RELEASE_EVIDENCE_FIELD_DIAGNOSTICS.md) | 新增 31-key projection、DUMP debug Activity、三包/launcher/Service preflight 和 5 executed + 3 NOT_RUN adapter | host software only；目标未执行、report 未 admitted |
| 2026-07-18 | [P9-W07a Release Evidence Envelope](docs/CENTRAL_BRAIN_RELEASE_EVIDENCE_FIELD_DIAGNOSTICS.md) | 新增 strict release identity、八类有序 diagnostic fact、stable report digest、GitHub privacy gate 和九组 JVM tests | metadata-only；target owner/report/Runtime/retest 未接；下一步 P9-W07b |
| 2026-07-18 | [P9-W06b Driver Safety Redacted Probe](docs/CENTRAL_BRAIN_DRIVER_SAFETY_ADMISSION.md) | 新增 27-key count/boolean projection、DUMP debug Activity、六组 JVM tests 和 no-install ADB adapter | host software only；目标 probe 未执行；不读取 vehicle scalar/owner reference/raw log；下一步 P9-W07 |
...
| 2026-07-18 | [P8-W01 Redacted Public Inventory](docs/CENTRAL_BRAIN_TARGET_CAPABILITY_DISCOVERY.md) | API 33 目标只读采集完成；仅发布 feature/service/command 聚合计数、隐私确认和内部 evidence reference | 不发布设备身份或原始 service；不等于 property mapping，P8-W01 mapping 与 P8-W02..W06 保持 `EXTERNAL_BLOCKED` |
...
P10-R1 冻结时的 Android 13 AIOS 软件需求已全部实现或由失败关闭接口覆盖，未分类仓库需求数为 0。
真实车辆/VHAL/SOA、Vendor NPU、可信 Safety/DMS/identity、量产签名/SELinux/OTA 和目标资格证据仍是外部输入，
不因软件完成而提升为 production。可执行 security campaign 按用户决策保持挂起。机器可读基线：
[`central_brain_android_software_completion_v1.json`](central-brain/contracts/central_brain_android_software_completion_v1.json)；
门禁：`tools/check_central_brain_android_software_completion.sh`。

`repository_software_requirements_complete=true`；`unclassified_repository_requirement_count=0`；
`security_requirement_suspended=true`；`hardware_accessed=false`；`production_ready=false`；
`target_hardware_validated=false`；`implementation_stage=P10-R1`。

此后新增的模型网关需求由 P7-R2/P7-R3 单独跟踪：P7-R2 开发网关已完成，P7-R3 量产 Provider/NPU 接入为
`PLANNED`。因此 `repository_software_requirements_complete=true` 仅是 P10-R1 冻结范围的历史合同值，不能用于声明
扩展后的模型量产范围已经完成。
