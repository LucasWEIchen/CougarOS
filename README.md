...
flowchart TB
  subgraph App["应用与 HMI"]
    ...
    HmiState["Immutable HMI state / reducer / lifecycle（P4-W02 已完成）"]
...
| Client2 基础演示 HMI 与 Demo HMI | 导航触发悬浮菜单、12 场景、typed Session/Event、immutable state/reducer、Java lifecycle、text-free process resume | Android 13 ARM64 应用层；尚无四阶段 renderer 和 HVAC/Seat 控制页 | `DEVELOPED` |
...
| 模块 | 最小剩余工作 | 阻塞或下一步 | 状态 |
| --- | --- | --- | --- |
| 场景解析与仿真编排 | P2 foundation、P3-W01..W09 Graph/recovery、P4-W01 Session/Event bridge 和 P4-W02 HMI reducer/lifecycle 已完成；仍需四阶段 shell 与 Runtime 场景编排闭环 | Stage 2 P4-W03 | `IN_PROGRESS` |
...
| `apk-labs/client2-central-brain/` | 座舱演示 HMI | `Client2ScenarioBridge`、immutable `CockpitHmiState`、唯一 `CockpitHmiReducer`、Java `CockpitControlCoordinator` 与 APK patch/build/test；下一步在同一 overlay 增加意图编排四阶段与 Effect 闭环 |
