...
require_text "$ARCH" 'ARCHITECTURE_DOCUMENT_BASELINE_READY'
...
require_text "$DETAIL" 'SOFTWARE_DESIGN_BASELINE_READY'
...
require_text "$INTERFACE" 'INTERFACE_DESIGN_BASELINE_READY'
...
for file in "$ARCH" "$DETAIL" "$INTERFACE"; do
  ...
  require_text "$file" 'p4_r7_design_published_on_main=true'
  require_text "$file" 'p4_r7_implementation_draft=true'
  require_text "$file" 'p4_r7_main_implementation_merged=false'
...
require_text "$README" 'P4-R7 design / implementation Draft #130'
require_text "$README" '双屏渲染恢复设计\nClient1 first / Client2 second\nimplementation Draft #130'

require_text "$ROADMAP" '版本：1.3'
require_text "$ROADMAP" 'ARCHITECTURE_BASELINE_READY / P4_R7_IMPLEMENTATION_DRAFT'
...
require_text "$BACKLOG" 'ARCHITECTURE_BASELINE_READY / P4_R7_IMPLEMENTATION_DRAFT'
require_text "$BACKLOG" '## P10-R1 文档与工作包收口'
require_text "$BACKLOG" '## P4-R7 双屏渲染、动态温区与车模旋转'

require_text "$P4_R7" 'DESIGN_PUBLISHED_ON_MAIN / IMPLEMENTATION_DRAFT_PR_130'
require_text "$P4_R7" '不表示对应代码已经合入 `main`'
