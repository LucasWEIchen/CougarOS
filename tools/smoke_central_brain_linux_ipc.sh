...
gate_ids = {item["gate_id"] for item in payload["mandatory_gates"]}
assert response["status"] == "ok", response
assert payload["operation"] == "event-subscription-activation-approval-decision-owner-handoff-evidence-acceptance-closure-readiness-decision-reviewer-assignment-audit-decision-rollup-closure-handoff-readiness-audit-decision-rollup-closure-blocker-matrix", response
assert payload["state"] == "contract-only-handoff-evidence-acceptance-closure-handoff-readiness-audit-decision-closure-blocked", response
assert payload["approval_decision_owner_handoff_evidence_acceptance_closure_readiness_decision_reviewer_assignment_audit_decision_rollup_closure_handoff_readiness_audit_decision_rollup_closure_blocker_matrix_active"] is True, response
assert payload["closure_blocker_matrix_complete"] is True and payload["closure_blocker_matrix_consistent"] is True, response
assert payload["source_closure_handoff_readiness_audit_decision_rollup_bound"] is True, response
assert payload["required_blocker_count"] == 10 and payload["open_blocker_count"] == 10 and payload["closed_blocker_count"] == 0, response
assert payload["assigned_reviewer_count"] == 0 and payload["unassigned_reviewer_count"] == 10, response
...
assert len(payload["blocker_items"]) == 10, response
for item in payload["blocker_items"]:
...
PY
SOA_EXTENSION_CLOSURE_OUTPUT="$(CENTRAL_BRAIN_IPC_SOCKET="$SOCKET_PATH" python3 "$ROOT_DIR/central-brain/bindings/linux/ipc/central_brain_ipc_client.py" soa-extension-closure-summary)"
...
PY
OBSERVABILITY_READINESS_OUTPUT="$(CENTRAL_BRAIN_IPC_SOCKET="$SOCKET_PATH" python3 "$ROOT_DIR/central-brain/bindings/linux/ipc/central_brain_ipc_client.py" observability-readiness)"
...
closure_chain = payload["event_subscription_activation_closure_chain_summary"]
stage_ids = {stage["stage_id"] for stage in closure_chain["stages"]}
assert payload["summary"]["event_subscription_activation_closure_chain_summary_active"] is True, response
assert payload["summary"]["event_subscription_activation_closure_chain_complete"] is True, response
assert payload["summary"]["event_subscription_activation_closure_chain_ready"] is False, response
assert payload["summary"]["event_subscription_activation_closure_chain_stage_count"] == 30, response
assert payload["summary"]["event_subscription_activation_closure_chain_android_linux_parity"] is True, response
assert payload["summary"]["event_subscription_activation_closure_chain_no_store_consistent"] is True, response
assert payload["summary"]["event_subscription_activation_closure_chain_no_side_effects_consistent"] is True, response
