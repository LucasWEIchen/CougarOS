...
SOFTWARE_DESIGN_CHECKER="$ROOT_DIR/tools/check_central_brain_software_detailed_design.sh"
...
for file in "$CONTRACT" "$PROFILE" "$DOC" "$ISSUE_FORM" "$ISSUE_CONFIG" \
    ...
    "$SOFTWARE_DESIGN_CHECKER" "$PRE_PUSH_HOOK"; do
...
bash -n "$RUNNER" "$PUBLICATION_CHECKER" "$ROOT_README_CHECKER" \
  "$SOFTWARE_DESIGN_CHECKER" "$PRE_PUSH_HOOK"
...
grep -Fq 'bash tools/check_central_brain_software_detailed_design.sh' "$WORKFLOW"
grep -Fq -- '- "README.md"' "$WORKFLOW"
grep -Fq -- '- "docs/CENTRAL_BRAIN_SOFTWARE_DETAILED_DESIGN.md"' "$WORKFLOW"
