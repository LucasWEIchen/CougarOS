...
if len(actual) != 37 or sum(item["kind"] == "INTERFACE" for item in actual) != 7:
    raise SystemExit("P9-W03c AIDL surface/interface count changed")
if sum(item["kind"] == "PARCELABLE" for item in actual) != 30:
    raise SystemExit("P9-W03c AIDL parcelable count changed")
if contract.get("aidl_surface_count") != 37 \
        or contract.get("aidl_interface_count") != 7 \
        or contract.get("aidl_parcelable_count") != 30:
...
for marker in [
    "publicAidlTreeMatchesSevenInterfacesThirtyParcelablesAndNamespaces",
...
for marker in \
  'AIDL_INTERFACE_COUNT = 7' \
  'AIDL_PARCELABLE_COUNT = 30' \
...
require_text "docs/CENTRAL_BRAIN_ARCHITECTURE_REQUIREMENTS.md" 'P9-W03c security boundary inventory trace'
require_text "docs/CENTRAL_BRAIN_INTERFACE_DESIGN.md" 'Android P9-W03c Security Boundary Inventory Contract'
require_text "docs/CENTRAL_BRAIN_SOFTWARE_ARCHITECTURE.md" 'P9-W03c security boundary inventory architecture'
require_text "docs/CENTRAL_BRAIN_COMPLETE_SOFTWARE_DEVELOPMENT_DESIGN.md" 'P9-W03c security boundary inventory detailed design'
require_text "docs/CENTRAL_BRAIN_DELIVERY_TARGETS.md" 'Android P9-W03c Security Boundary Inventory'
require_text "docs/CENTRAL_BRAIN_DRIVER_INTERFACE_SUPPORT.md" 'P9-W03c Security Boundary Inventory Driver/HAL Boundary'
require_text "docs/CENTRAL_BRAIN_ARCHITECTURE_DEVIATIONS.md" 'DEV-090 P9-W03c static inventory and debug probe availability are not target fuzz evidence'
require_text "docs/CENTRAL_BRAIN_ARCHITECTURE_ISSUES.md" 'ISSUE-050 P9 complete security fuzz evidence is unavailable'
require_text "docs/CENTRAL_BRAIN_ROADMAP.md" 'P9-W03c Security Boundary Inventory progress'

printf '%s\n' \
  'Central Brain Android security boundary inventory check passed' \
  'security_aidl_parcel_inventory_complete=true' \
  'security_aidl_interface_count=7' \
  'security_aidl_parcelable_count=30' \
  'security_aidl_surface_count=37' \
